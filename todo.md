...
- [ ] Save a checkpoint after synchronization so the user can publish the current version manually.
- [ ] Push the matching code to the configured GitHub repository before publication.

## Database Plus: הכנה לפרסום

- [ ] Review the pending Database Plus changes and exclude unrelated work before checkpointing.
- [ ] Save a checkpoint for the Database Plus payment-route update and push the matching source to GitHub.
- [ ] Publish the saved project version through the Manus interface and validate the same-origin Production payment handoff.

## Database Plus: ניתוב בדיקה דרך Production

- [ ] Point the Database Plus preview payment action to the approved hilitcaspi.com recurring-mandate endpoint.
- [ ] Keep the browser request free of Grow identifiers and card information.
- [ ] Verify the preview button receives and opens the Sandbox hosted form returned by Production.

## Database Plus: בדיקה דרך שרת Production

- [ ] Identify the publicly deployed Production endpoint that can proxy the 1 ₪ Sandbox mandate request.
- [ ] Send one hosted-form creation request through that Production endpoint without card data or transaction completion.
- [ ] Record only the HTTP result and whether Grow returned a hosted URL.

## Database Plus: התאמת multipart ל־Grow

- [ ] Compare the exact multipart framing, headers, field order, and request origin of the working Grow Swagger call and Database Plus.
- [ ] Update Database Plus to match the verified request protocol without exposing credentials or payment data.
- [ ] Verify the returned hosted URL from the page flow.

## Database Plus: כתובת טופס חסרה

- [ ] Inspect the latest recurring-mandate endpoint response and safely identify why no hosted URL was returned.
- [ ] Correct the response mapping or Grow request so the client receives the hosted Sandbox URL.
- [ ] Verify the redirect from Database Plus to the returned Grow form.

## Database Plus: ניתוח ניסיון החי האחרון

- [ ] Inspect the Database Plus Grow request recorded in the current minute and identify the precise runtime branch and Grow response.
- [ ] Compare the live runtime payload with the verified Postman payload without logging credentials, customer data, or hosted URLs.
- [ ] Apply and verify the correction to the active payment path.

## Database Plus: אבחון נתיב חי

- [ ] Capture a safe runtime marker proving whether the live Plus button calls the dedicated Sandbox recurring-payment request or the legacy payment path.
- [ ] Remove the legacy Plus payment path if it is still handling the click, then verify the hosted Sandbox URL is used.

## Database Plus: אימות מול דוגמת Postman

- [ ] Compare the active server-side Sandbox identifiers with the supplied working Postman pair without logging their values.
- [ ] Record a safe field-by-field comparison of the Database Plus request and the working Postman request.
- [ ] Correct any mismatch and verify the hosted Sandbox URL is returned from the Database Plus flow.

## Database Plus: התאמה לדוגמת הוראת קבע תקינה

- [ ] Switch the Plus recurring request to multipart/form-data using the active Sandbox identifiers.
- [ ] Include chargeType, invoiceName, fullName, phone, and email fields exactly as in the verified Grow Sandbox request.
- [ ] Return the hosted Sandbox URL and open it through the existing Plus payment experience without exposing payment identifiers.

## Database Plus: חזרה ל־Grow Sandbox

- [x] Change only the Grow environment and identifiers for Database Plus to the supplied Sandbox configuration.
- [x] Keep the existing Plus payment component, customer fields, consent UX, and 1 ₪ test amount unchanged.
- [ ] Verify Sandbox hosted-checkout opening without entering card details.

## Database Plus: מסך חיוב חודשי של Grow

- [ ] Verify the pageCode currently used for Database Plus is activated in Grow for recurring monthly charges.
- [ ] Replace the Database Plus test configuration with the active recurring-payment pageCode once confirmed.
- [ ] Re-run the 1 ₪ Production test and confirm the recurring hosted payment screen opens.

## Database Plus: אישור גיל

- [x] Fix the age-confirmation validation so a checked Database Plus age box is reliably recognized on payment click.
- [ ] Verify the payment button does not show the age warning after the age box is checked.

## Database Plus: כשל פתיחת Grow Production מהכפתור

- [ ] Compare the browser-triggered Database Plus request against the direct successful Production request.
- [x] Return Database Plus to the established Grow checkout path and reuse the existing Production identifiers for the approved 1 ₪ test.

## Database Plus: קישור תצוגה

- [ ] Find and verify the current active preview URL for the Database Plus route after the reported load failure.

## Database Plus: בדיקת Production עם מזהים פעילים

- [x] Reuse the active production userId and pageCode from the existing working Grow payment flow for the approved 1 ₪ recurring-payment test.
- [x] Confirm that no separate Plus production page code is required for the test form.

## Database Plus: בדיקת ייצור מאושרת

- [ ] Create one Production Grow recurring-payment hosted form for a 1 ₪ test after the owner’s explicit confirmation.
- [ ] Do not enter card details or complete the transaction; report only whether the hosted form is created.

## Database Plus: התאמה לקריאת Grow הפעילה

- [ ] Inspect the working Grow payment creation flow and its proxy behavior field by field.
- [x] Reuse the working Grow server/proxy request conventions for the Database Plus recurring mandate, changing only the recurring-specific fields.
- [ ] Verify the 1 ₪ Sandbox test through the same established request path.

## Database Plus: תמיכה בשם עברי ב־Grow Sandbox

- [ ] Identify the compatible ASCII-safe encoding for Hebrew name fields in the Grow Sandbox recurring-payment request.
- [ ] Encode the outgoing name field without changing the stored customer name or exposing it in logs.
- [ ] Verify hosted-checkout creation with a Hebrew test name.

## Database Plus: שגיאת פתיחת טופס Sandbox

- [ ] Inspect the server-side Grow Sandbox response for the Database Plus test form and identify its safe error code.
- [ ] Correct the hosted-form request based on the returned Grow error, without exposing identifiers or card data.
- [ ] Verify that the 1 ₪ Database Plus Sandbox test redirects to Grow hosted checkout.

## Database Plus: פתיחת בדיקת Sandbox

- [x] Enable a clearly marked Sandbox-only test entry point on Database Plus when no personal profile link is present.
- [x] Keep the production eligibility gate intact for personal Database Plus subscription links.
- [ ] Verify the enabled Sandbox test opens the 1 ₪ Grow hosted form without collecting card data on the website.

## Database Plus: נוסח הכפתור

- [x] Change the inactive Database Plus CTA label to "נדרש קישור אישי מהפרופיל האישי" and verify its display.

## Database Plus: תקלה בטעינת תצוגה

- [ ] Diagnose why the Database Plus preview route does not load.
- [ ] Restore the preview server and verify the Database Plus route is reachable.

## Database Plus: בדיקת חיוב Sandbox

- [x] Change the Database Plus recurring-mandate Sandbox amount from 99 ₪ to 1 ₪ for the test only.
- [x] Display the 1 ₪ test amount consistently in the Database Plus test flow without changing the production subscription amount.
- [ ] Verify the 1 ₪ Sandbox hosted-payment handoff and document the production rollback value of 99 ₪.

## Database Plus: מנוי בהוראת קבע

- [x] Locate the Database Plus page, its subscription call-to-action, and the existing payment route.
- [x] Add a Hebrew monthly-mandate summary and explicit consent flow inside the Database Plus experience.
- [x] Connect the Database Plus subscription action to a server-only Grow Sandbox hosted-checkout request.
- [ ] Keep existing Database Plus registration and payment flows unchanged outside the new recurring-mandate entry point.
- [ ] Add Vitest coverage and validate the responsive secure payment handoff.
