...
- [x] להריץ pnpm test, pnpm check, pnpm build ו־git diff --check
- [ ] לפרסם את תיקון ההסרה והסינון ולסנכרן GitHub
- [ ] לפשט את ניסוי הקהל הקר כך ששאלון DNA נשאר דף הכניסה המרכזי
- [ ] להגדיר ניסוי קטן וברור של Objective Leads מול Objective Sales עם אותו DNA ואותו קהל
- [ ] להסביר מדוע לא מערבבים בניסוי הראשי עמודי נחיתה שונים, קהלים שונים ומטרות שונות יחד
- [ ] להגדיר ריטרגטינג נפרד רק לאחר מילוי DNA, עם יעד מכירה לבאנדל או למאגר
- [ ] להגדיר תקציב קטן, משך קצר, UTM ומדדי הכרעה לכל ניסוי בלי להפעיל קמפיין
