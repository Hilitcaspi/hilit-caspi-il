...
- [~] ארכיון: בקשת לקוח היסטורית שאינה פעילה; טיפול חדש דורש בקשה ואימות עדכניים
- [~] ארכיון: בדיקת מסירה היסטורית שאינה פעילה
- [~] ארכיון: כלל בטיחות נשמר במדיניות העבודה ואינו משימת ביצוע פתוחה
- [~] ארכיון: שליחה היסטורית לא תבוצע מחדש ללא בקשה עדכנית
- [~] ארכיון: אימות מסירה היסטורי אינו משימה פעילה

## בדיקת קבלת מיילים ומייל בדיקה דרך ערוץ ההתאמות (30.8.2026)

- [~] ארכיון: בקשת לקוח היסטורית שאינה פעילה; טיפול חדש דורש בקשה ואימות עדכניים
- [~] ארכיון: בדיקת דיוור היסטורית שאינה פעילה
- [~] ארכיון: כלל בטיחות נשמר במדיניות העבודה ואינו משימת ביצוע פתוחה
- [~] ארכיון: מייל בדיקה היסטורי לא יישלח ללא בקשה עדכנית
- [~] ארכיון: אימות ספק היסטורי אינו משימה פעילה
...
- [~] ארכיון: טיפול היסטורי ברשומת לקוח; שינוי חדש דורש בדיקה ובקשה עדכניות
- [~] ארכיון: שליחת אישור מחדש אינה משימה פעילה
...
- [~] ארכיון: בקשת התאמה היסטורית שאינה פעילה; שינוי חדש דורש בקשה ואימות עדכניים
- [~] ארכיון: בדיקת סטטוס היסטורית אינה משימה פעילה
- [~] ארכיון: לא יבוצע שחרור התאמה ישנה ללא בקשה עדכנית
- [~] ארכיון: לא ישונה סטטוס לקוחות ללא בקשה עדכנית
- [~] ארכיון: אימות חסימות היסטורי אינו משימה פעילה
...
- [~] רעיון עתידי שממתין לבקשה ואישור מפורשים; אינו משימה פעילה
- [~] רעיון עתידי: Opt-in נפרד והפעלה אוטומטית יבוצעו רק לאחר בקשה חדשה ואישור
- [~] רעיון עתידי: הרחבת אזור Boost אינה משימה פעילה
- [~] רעיון עתידי: תזכורות Boost אינן משימה פעילה
...
- [x] התקבלו החלטות ואישורים לשעה, נמענים, Meta ויעדים במסגרת המשך המשימה
...
- [x] הוצגה הודעת דוגמה והתקבל אישור מפורש לנמענים, שעה, נוסח והפעלה
- [x] התזמון היומי הופעל לאחר האישור ונבדק בדוחות חצות
...
## הסרת מחיר המוצרים בנפרד מעמוד באנדל החג — 03.09.2026

- [x] לאתר את כרטיס המחיר האמצעי „כיום בנפרד 697 ₪” בעמוד באנדל החג
- [x] להסיר רק את הכרטיס האמצעי ולהשאיר את מחיר החג 399 ₪ ואת השווי המקורי 1,245 ₪
- [x] לאזן את פריסת שני המחירים שנותרו בדסקטופ ובמובייל בלי לשנות את פלואו התשלום
- [x] להוסיף בדיקת רגרסיה שמוודאת שהמחיר האמצעי אינו מוצג ושנותרו שתי עמודות בלבד
- [x] להריץ בדיקות, TypeScript, build ו־git diff --check: 70 קובצי בדיקה ו־335 בדיקות עברו
- [x] לבדוק את העמוד בדסקטופ ובמובייל
- [x] לפרסם את השינוי ולסנכרן GitHub — גרסה 12b9b69a

## צמצום המלל והמספרים במודעות באנדל החג — 03.09.2026

- [x] למפות את כל מודעות באנדל החג הקיימות בפורמטי פיד 1:1 וסטורי 9:16
- [x] להגדיר למודעות מכירתיות מחיר יחיד וברור: 399 ₪ במקום 1,245 ₪
- [x] להסיר מהמודעות המכירתיות מחיר 697 ₪, מחירי מוצרים בודדים, סכומי חיסכון ומספרים כפולים
- [x] להשאיר מודעות טיזר ללא מחיר ולצמצם מלל שאינו חיוני
- [x] לערוך 14 מודעות תוך שמירת תמונות הילית, המיתוג, הצבעוניות והקומפוזיציה שאושרו
- [x] לבדוק בכל מודעה את האיות, המחיר, קריאות הטקסט, הפורמט ואזורי הבטיחות של Meta
- [x] להכין שבעה קובצי פיד 1:1 ושבעה קובצי סטורי 9:16, בשני קובצי ZIP נפרדים ועם מניפסט שימוש

## ניוזלטר DNA לבאנדל החג עם 10% הנחה — 03.09.2026

- [x] לבדוק את מחיר באנדל החג, מנגנון הקופונים וזרימת התשלום לפני יצירת ההטבה
- [x] ליצור קוד HOLIDAY10 כהנחה של 10% לבאנדל בלבד, ללא Boost, Plus או מוצרים אחרים, בתוקף עד 6.9.2026 בשעה 23:59 לפי שעון ישראל
- [x] לאמת דרך API התשלום את המחיר הסופי 359.10 ₪ ואת החלת הקוד האוטומטית לאחר הזנת המייל
- [x] לכתוב ולעצב ניוזלטר מכירתי המבוסס על המסר „שאלון ה־DNA היה נקודת הפתיחה” ועל תוכן עמוד הבאנדל
- [x] להוסיף קישור מדויק לעמוד באנדל החג עם UTM ייעודי וקישור הסרה חתום אישי ב־HTML ובטקסט
- [x] לבנות קהל של משלימי DNA בלבד, ללא רוכשי הבאנדל, מסירי דיוור, כתובות חסומות ופרופילים לא פעילים
- [x] לבצע בדיקת ספירה מצרפית: 8,151 נמענים זכאים לאחר כל ההחרגות, ללא חשיפת פרטים אישיים
- [x] להפיק תצוגת ניסיון ולבדוק נושא, preheader, HTML, טקסט, קופון, מחיר, קישורים ודף נחיתה במובייל
- [x] לשלוח את הניוזלטר היום לקהל המאושר בלבד: 8,152 נשלחו, 8,089 ללא bounce מיידי, 62 soft bounce ו־1 hard bounce
- [x] להריץ בדיקות, TypeScript, build ו־git diff --check: 72 קובצי בדיקה ו־340 בדיקות עברו
- [x] לפרסם את התשתית ולסנכרן GitHub — גרסה 833bb8fd

## הסרת דיאנה טולצינסקי מהמאגר וחסימת דיוור — 03.09.2026

- [x] לאמת את רשומת הפרופיל וכל התאמה, חברות Boost או ישות פעילה המקושרת אליה בלי לשנות נתונים
- [x] להחיל מיד חסימת שיווק ו־CRM; לא נמצאו מיילים pending או processing לביטול
- [x] לסגור 9 התאמות פתוחות ולהסיר את הפרופיל ממאגר ההתאמות בלי להשאיר מועמדות עתידית
- [x] להסיר חברות Boost פעילה ולבטל 2 בקשות Boost; לא נמצאה חברות Plus
- [x] לשמור רשומת חסימה מינימלית ב־CRM ובפרופיל הלא פעיל כדי שהכתובת לא תיכנס מחדש למסעות דיוור או קמפיינים
- [x] לאמת לאחר הפעולה: אפס פרופילים פעילים, אפס התאמות פתוחות, אפס בקשות Boost פתוחות ואפס מיילים ממתינים
- [x] למסור סיכום הסרה ללא חשיפת פרטים אישיים מיותרים

## עצירת ההסרה ושחזור דיאנה למאגר לצורך בדיקה — 03.09.2026

- [x] לעצור פעולות הסרה נוספות ולהשאיר את כל היסטוריית ההתאמות הקיימת ללא מחיקה
- [x] למפות את כל השדות והסטטוסים ששונו בפעולת ההסרה ואת ערכי השחזור הזמינים
- [x] לשחזר את הפרופיל למאגר הפעיל בלי להפעיל מחדש שיווק או תורי מייל
- [x] להוסיף למנוע ההתאמות הרגיל סינון consentMatchmaking כדי שפרופיל פעיל וחסום לא יקבל התאמות אוטומטיות
- [x] לשמור את כל 11 רשומות ההתאמה כהיסטוריה; תשע הרשומות שהיו פתוחות נשארו סגורות כדי לא לפתוח מחדש קשרים או לשלוח הודעות לצדדים
- [x] לשמור את היסטוריית חברות Boost ושתי הבקשות במצב removed/cancelled, בלי להפעילן או לשלוח הודעות
- [x] לשחזר את טוקן הגישה האישי המקורי; כתובת התמונה שנמחקה לא הייתה זמינה לשחזור בטוח ולכן נשארה חסרה
- [x] לאמת שהפרופיל וכל 11 ההתאמות קיימים במערכת ושאין מיילים pending או processing
- [x] למסור להילית פירוט מדויק של מה שוחזר ומה נשאר חסום בזמן הבדיקה

## בדיקת והרצת התאמות מחודשת לפרופיל כינר עשור — 03.09.2026

- [x] לאמת את הפרופיל, סטטוס המאגר, השלמת 25 תשובות השאלון, DNA והעדפות החיפוש של כינר
- [x] לנתח מדוע אפשרויות ההתאמה הקיימות מדורגות מתחת ל־80%: הציון החישובי הגבוה ביותר במאגר כרגע הוא 79%
- [x] לבדוק 675 מועמדות פעילות עם התאמה הדדית למגדר; רכיב השאלון הכללי הוא הגורם העיקרי שמוריד את הציון, בעוד DNA, שלב חיים ודתיות גבוהים
- [x] להריץ מחדש את אלגוריתם ההתאמה עבור כינר בלבד בלי לשלוח מייל, WhatsApp או SMS
- [x] לשמור ב־CRM 6 אפשרויות התאמה חדשות בלבד, לאחר החרגת כל הזוגות הקיימים ומועמדות ללא שאלון מלא
- [x] לאמת את טווח הציונים החדש: 76%, 74%, 73%, 73%, 72%, 72%; אין כרגע אפשרות מעל 80%
- [x] לוודא שלא נשלחה הודעה, לא נוצר חיוב ולא הופעל Boost
- [x] למסור סיכום הסיבה והתוצאות בלי לחשוף פרטי קשר של מועמדות

## הסרת מאיה ורד מהמאגר וחסימת מיילי התאמה — 03.09.2026

- [x] לאמת את רשומת מאיה וכל התאמה, Boost, Plus או ישות פעילה המקושרת אליה
- [x] להחיל חסימת שיווק, משוב ומיילי התאמה; לא נמצאו מיילים pending או processing
- [x] להוציא את הפרופיל ממאגר ההתאמות ולבטל הסכמות התאמה ושיתוף מידע
- [x] לסגור 6 התאמות פתוחות ולהחזיר את הצדדים האחרים למאגר בלי למחוק את 7 רשומות ההיסטוריה
- [x] לאמת שלא קיימות חברות Boost או Plus פעילות ולא קיימות בקשות Boost פתוחות
- [x] לשמור רשומת חסימה מינימלית ב־CRM ובפרופיל הלא פעיל כדי למנוע כניסה מחדש לדיוור או למסעות התאמה
- [x] לאמת אפס פרופילים פעילים, אפס התאמות פתוחות, אפס מיילים ממתינים וחסימה פעילה במנגנון השיווק
- [x] למסור סיכום הסרה ללא חשיפת פרטים אישיים מיותרים

## תיקון מגדר ופרטים חסרים בפרופיל מושפע — 03.09.2026

- [x] לאמת את הרשומה המושפעת במאגר, ב־CRM, בתוצאות DNA, בתשלום ובטפסי ההרשמה
- [x] לזהות את שורש התקלה: webhook התשלום הקדים את שמירת הטופס המלא ויצר skeleton עם ברירת מחדל שגויה
- [x] לתקן את המגדר ל„אישה” על בסיס מקורות CRM ו־DNA מאומתים בלבד
- [x] להשאיר את העדפת המגדר ושדות שלא נמצאו במקור מאומת כמסומנים חסרים, בלי להסיק או להמציא מידע
- [x] לשמור פרופיל מלא, לא משולם ולא פעיל לפני פתיחת Grow, ולחסום פתיחת checkout אם השמירה נכשלת
- [x] למנוע מייל, GA4, קידום CRM והפעלת פרופיל בשמירת הטיוטה; להשאיר את האישור והמייל ל־webhook תשלום מאומת
- [x] לעדכן retry של טיוטה קיימת בלי ליצור כפילות ולשמור מסלולי free token וקוד הזמנה בנפרד
- [x] להוסיף בדיקות התנהגותיות וסינתטיות לטיוטה, retry, פרופיל שכבר שולם, free token והפעלת Grow ללא דריסת פרטים
- [x] לבדוק את עמוד ההצטרפות בדסקטופ ובמובייל בלי לשלוח טופס ובלי לפתוח תשלום
- [x] לוודא שלא נשלחה ללקוחה הודעה במהלך הבדיקה והתיקון
- [x] להריץ 77 קובצי בדיקה ו־354 בדיקות, TypeScript, build ו־git diff --check בהצלחה
- [x] לפרסם ולסנכרן GitHub לאחר השלמת כל שערי השחרור

## העברת שינויי Database Plus מהמשימה הקודמת — 03.09.2026

- [x] לאתר ב־GitHub ובגרסאות הפרויקט את מקור שינויי Database Plus מהמשימה הקודמת
- [x] לשמור את מזהי Grow Sandbox שסופקו כסודות שרת בלבד ולא להכניס לקוד או ל־GitHub פרטי אדם מהדוגמה
- [x] להפעיל את הזרימה המועברת במצב Sandbox של 1 ₪ בלבד ולהבהיר במסך שאין זה חיוב הייצור של 99 ₪
- [x] להשוות את השינויים לגרסה הפעילה ולבודד רק קוד ותוכן שעדיין חסרים
- [x] למזג את שינויי Database Plus בלי לדרוס את תיקון מרוץ ההרשמה או מנגנוני הפרטיות והתשלום
- [x] להוסיף או לעדכן בדיקות Vitest לכיסוי השינויים שהועברו
- [x] לבדוק את מסכי Database Plus בדסקטופ ובמובייל בלי להזין לקוח ובלי לפתוח תשלום
- [x] להריץ 80 קובצי בדיקה ו־364 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקת Grow החיה אומתה פעם אחת והוגדרה כאופט־אין כדי לא להפעיל חסימת WAF בבדיקות חוזרות
- [x] לפרסם את הגרסה ולסנכרן את אותו checkpoint ל־GitHub

## פתיחת Database Plus לכל מי שנכנס לקישור — 03.09.2026

- [x] להסיר מהעמוד, מהתקנון ומהאזור האישי את הנוסח ש־Plus זמין רק לחברים פעילים במאגר
- [x] להסיר מיצירת התשלום את דרישת פרופיל המאגר ואת התאמת האימייל והטלפון לפרופיל קיים
- [x] ליצור רשומת checkout intent בטוחה ואידמפוטנטית גם כשאין עדיין single קיים
- [x] לעדכן את webhook כך שרכישת Plus ללא פרופיל נשמרת כ־paid pending ונשלח קישור אישי להשלמת הפרופיל
- [x] לקשר ולהפעיל אוטומטית את Plus לאחר השלמת הפרופיל, בלי placeholder ובלי תשלום נוסף
- [x] לשמור את שלוש הסכמות Plus כתנאי לפתיחת התשלום
- [x] להוסיף בדיקות Vitest למסלול ציבורי ללא חברות קודמת, plus_ref חתום, fulfillment ומניעת כפילות
- [x] לבדוק את עמוד Database Plus בדסקטופ ובמובייל בלי להזין פרטים ובלי לבצע תשלום
- [x] להריץ 81 קובצי בדיקה ו־369 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקת Grow החיה נשארה opt-in
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## תקלה ביצירת תשלום Plus עקב שאילתת plus_checkout_intents — 03.09.2026

- [x] לבדוק בלוגי הייצור את שגיאת השאילתה ולאשר שהיישום פונה למסד LEGACY בעוד שהמיגרציה הוחלה קודם במסד המנוהל
- [x] לאמת את מבנה טבלת plus_checkout_intents מול סכמת Drizzle והמיגרציה
- [x] להחיל במסד LEGACY הפעיל יצירת טבלה לא־הרסנית עם 19 עמודות וארבעת האינדקסים הנדרשים
- [x] להוסיף בדיקת רגרסיה שמוודאת שמקור מסד הנתונים הפעיל והמיגרציה של Plus נשארים מתועדים ועקביים
- [x] להריץ בהצלחה מול מסד LEGACY את שאילתת ה־status המדויקת שנכשלה, עם כתובת סינתטית ובלי לפתוח Grow או ליצור תשלום
- [x] להריץ 82 קובצי בדיקה ו־370 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקה חיה אחת נשארה opt-in
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## מעבר Database Plus מ־Grow Sandbox לסליקת ייצור — 03.09.2026

- [x] לאתר את כתובת Grow, ה־userId וה־pageCode של סליקות הייצור הקיימות בלי לחשוף ערכים
- [x] לוודא בבקשת ייצור סינתטית שה־pageCode הקיים מקבל chargeType=1 וסכום 99 ₪ ומחזיר authCode ו־processToken
- [x] לחבר את Database Plus לכתובת הייצור ולמזהים בצד השרת בלבד
- [x] להסיר מהעמוד את הודעת Sandbox ולעדכן את סכום החיוב בפועל ל־99 ₪ לחודש
- [x] לשמור עדיפות ברורה ל־pageCode ייעודי ל־Plus אם יחובר בעתיד
- [x] להוסיף בדיקות רגרסיה להפרדת Sandbox וייצור, ל־chargeType=1 ולסכום 99 ₪
- [x] לאמת יצירת תהליך ב־Grow ייצור עד לקבלת authCode ו־processToken בלבד, בלי לפתוח או להשלים תשלום
- [x] להסיר authCode ו־processToken מתגובות Grow שנכתבות ללוגים
...
## הסתרת Database Plus וטיפול בניסיונות חיוב של 1 ₪ — 03.09.2026

- [x] לחסום את `/database-plus`, `/terms/plus` ו־`/thank-you/plus` ולהציג 404 בכל קישור ישיר
- [x] לחסום בצד השרת יצירת תהליך תשלום Plus גם אם מישהו קורא ישירות ל־API
- [x] להסיר קישורי רכישה גלויים מהאזור האישי ולחסום שליחת הזמנות Plus חדשות
- [x] לחסום גם את עמוד התודה כך שאינו מציג תוכן Plus או מאפשר להתחיל תשלום
- [x] להוסיף בדיקות רגרסיה לחסימת UI ושרת לפני יצירת intent או קריאת Grow
- [x] לאמת בדפדפן שכל שלושת הנתיבים מציגים 404 ואינם חושפים תוכן או תשלום
- [x] לנסח הודעת WhatsApp שמבהירה שנגבה 1 ₪ בלבד ומציעה השלמת תשלום או זיכוי, בלי לשלוח אותה
- [x] להריץ 82 קובצי בדיקה ו־366 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקה חיה אחת נשארה opt-in
- [x] לפרסם מיד ולסנכרן את אותו checkpoint ל־GitHub

## שינוי אסטרטגיית באנדל החג והמודעות — 05.09.2026

- [x] לנתח 2,239 אירועי צפייה בעמוד החג, התחלות תשלום ורכישות מצרפיות בלי לחשוף פרטי לקוחות
- [x] לנתח את תוצאות ניוזלטר DNA ולאשר שהיו 349 אירועי UTM אך לא נוצרה רכישה מלאה של הבאנדל
- [x] לבנות הצעה חדשה שמעמידה את מאגר ההיכרויות בפרונט ואת המדריך והקורס כתוספת ערך
- [x] לנסח שלושה קונספטים חזקים שמייצרים עניין בלי להבטיח התאמה מיידית שאינה מובטחת
- [x] להגדיר קהלים, זוויות, כותרות, תיאורים וקריאות לפעולה לניסוי Sales קטן בן ארבעה ימים
- [x] ליצור שש מודעות חדשות: שלושה קונספטים בפיד 1:1 ובסטורי 9:16 בהתאם למיתוג החג
- [x] לבדוק שפה, קריאות, פרטיות ואיסור טענות או עדויות מומצאות בכל ששת הנכסים
- [x] למסור את המודעות ותוכנית הניסוי בלי להפעיל קמפיין, לשנות את העמוד או לשלוח דיוור

## עיצוב מחדש למודעות באנדל החג לפי הקמפיין המקורי — 05.09.2026

- [x] לצמצם כל מודעה לכותרת אחת, שורת חיזוק קצרה וקריאה לפעולה בלבד
- [x] להחליף את ניסוח „גלילה” ב־„החלקה ימינה ושמאלה” ולהימנע ממסרים לא ברורים
- [x] לשחזר את השפה החזותית הכהה, הדרמטית והיוקרתית של קמפיין החג המקורי והרפרנסים
- [x] להסיר מחיר מהקריאייטיבים ולשמור את המאגר כמוקד ההצעה
- [x] ליצור שלושה קונספטים מתוקנים בפיד 1:1 ובסטורי 9:16
- [x] לבדוק קריאות, עברית, מיעוט טקסט, זהות מותג ואזורי בטיחות בכל ששת הנכסים
- [x] למסור את הנכסים המתוקנים בלי להפעיל קמפיין או לשנות את האתר

## עיצוב מחדש למודעות באנדל החג: בהיר, חגיגי וברור — 05.09.2026

- [x] להשתמש במבנה המדויק של קמפיין המאגר המצורף: תמונה גדולה, כותרת צרה, שורת הסבר וכפתור קטן
- [x] לבסס את המסרים על „מאגר הרווקים החכם בישראל”, „התאמה חכמה עם עין אישית” ו„אפשרויות חדשות להכיר”
- [x] להציב את „מאגר הרווקים החכם בישראל” במפורש בכל מודעה, ככותרת או כשורת חיזוק בולטת
- [x] לנסח שלושה הוקים קצרים וברורים שקשורים ישירות להצטרפות למאגר
- [x] ליצור שפה חזותית אחידה עם רקע לבן־קרמי, פרחים חומים ואווירת חג
- [x] להשתמש באותו סגנון פונט, משקל והיררכיה בכל ששת הנכסים
- [x] להשאיר את המחיר מחוץ למודעות ולשמור על כמות טקסט מצומצמת אך מובנת
- [x] ליצור שלושה קונספטים בפיד 1:1 ובסטורי 9:16
- [x] לבדוק עברית, בהירות, אחידות, קריאות ואזורי בטיחות בכל ששת הנכסים
- [x] למסור את הסדרה המתוקנת בלי להפעיל קמפיין או לשנות את האתר

## מודעות חג חומות עם מסר היכרויות ו־CTA ממוסגר — 05.09.2026

- [x] לחזור לנראות החומה העמוקה, הפרחים הלבנים והפונט הצר של הרפרנס האחרון
- [x] להציב את „תקופת החגים היא בדיוק הזמן להיכרויות חדשות” כמסר המרכזי
- [x] לחזק „להכיר בדרך שונה” ו„מאגר הרווקים והרווקות החכם והגדול בישראל”
- [x] להחליף את ה־CTA עם הקו התחתון בכפתור ממוסגר שמנת־צהוב שנבחר לנראות חג יוקרתית
- [x] ליצור שלושה קונספטים בפיד 1:1 ובסטורי 9:16 עם כמות טקסט מצומצמת וברורה
- [x] לשמור את „השאלון היה רק ההתחלה” מחוץ לסדרה ולתעד אותו לקמפיין חם נפרד
- [x] לבדוק עברית, אחידות הפונט, נראות הכפתור, קריאות ואזורי בטיחות בכל ששת הנכסים
- [x] למסור את הסדרה בלי להפעיל קמפיין או לשנות את האתר

## גיוון מסרי מודעות החג ואימות הוכחה חברתית — 05.09.2026

- [x] לאמת 181 התאמות היסטוריות שבהן שני הצדדים אמרו כן, ולהשתמש בניסוח „יותר מ־180 התאמות” בלי לקרוא להן זוגות
- [x] לבדוק עדויות ולאשר שאין כרגע עדות עם הרשאה מפורשת למודעות ממומנות; לא להמציא ציטוט או סיפור הצלחה
- [x] לבנות שלוש זוויות שונות: הוכחה מצרפית, בדיקה אישית של הילית, ומדע ופרטיות מעבר לתמונה
- [x] לכתוב לכל זווית טקסט מלווה שמסביר מה שונה במאגר ואיך התהליך עובד
- [x] לעדכן את שלושת הקריאייטיבים החומים בפיד ובסטורי כך שלכל אחד מסר אחר
- [x] לשמור על פרחים לבנים, פונט צר וכפתור CTA ממוסגר בכל שש הגרסאות
- [x] לבדוק שאין נתון, עדות, הבטחה או טענה שאינם מבוססים ומאושרים
- [x] למסור את הסדרה והטקסטים בלי להפעיל קמפיין או לשנות את האתר

## שחזור תבנית מודעת החג ואימות נתון 1,600 — 05.09.2026

- [x] לאמת 6,112 רשומות של אפשרויות התאמה ולנסח באופן שמרני „יותר מ־1,600 אפשרויות התאמה כבר נבחנו במאגר”, בלי להציג אותן כזוגות או כהתאמות הדדיות
- [x] לשחזר את התבנית המצורפת עם תמונה גדולה של הילית, כותרת גדולה, מעט טקסט, רקע חום ופרחים לבנים
- [x] לשלב מודעה נפרדת עם „יצרתי במיוחד בשבילכם הטבה לחג” ו„נכנסים עכשיו כדי להתחיל את השנה הזו אחרת”
- [x] ליצור שתי גרסאות פיד 1:1 ושתי גרסאות סטורי 9:16 בלי להקטין את התמונה או את הטקסט
- [x] לשמור על CTA ממוסגר ועל הפונט הצר והאחיד של הרפרנס
- [x] לבדוק עברית, גודל, קריאות, דיוק הנתון ואזורי בטיחות בכל ארבע המודעות
- [x] למסור את המודעות בלי להפעיל קמפיין או לשנות את האתר

## סבב סופי לבאנדל החג: מגיעים מוכנים לזוגיות — 05.09.2026

- [x] לזקק מסר קריאייטיב קצר שמחבר בין השנה החדשה, שינוי אישי והגעה מוכנים לזוגיות
- [x] לכתוב טקסט Meta מלווה שמסביר למה נוצר הבאנדל ומה מקבלים בו בלי להעמיס על המודעה
- [x] ליצור קריאייטיב פיד 1:1 וסטורי 9:16 בנראות החומה המדויקת של הרפרנס האחרון
- [x] לשמור על תמונה גדולה של הילית, טיפוגרפיה צרה, מעט טקסט וכפתור ורוד בולט
- [x] לבדוק עברית, אחידות, קריאות, אזורי בטיחות ופוטנציאל המרה
- [x] למסור את הסבב הסופי בלי לשנות את האתר או להפעיל קמפיין

## סבב סופי לבאנדל החג: מגיעים מוכנים לאהבה — 05.09.2026

- [x] לנסח מסר מרכזי חד שמחבר את החגים, השינוי האישי והגעה מוכנים להיכרות
- [x] לכתוב טקסטים מלווים שמסבירים למה נוצר הבאנדל ומה שונה בו
- [x] ליצור סדרת פיד 1:1 וסטורי 9:16 בנראות חומה חגיגית לפי הרפרנס האחרון
- [x] לשמור על תמונה גדולה של הילית, פונט צר, מעט טקסט וכפתור ורוד ברור
- [x] לבדוק עברית, אחידות, אזורי בטיחות וקריאות בכל הנכסים
- [x] למסור את הסבב הסופי בלי לשנות את האתר או להפעיל קמפיין

## תיקון טיפוגרפיה ופריסת טקסט לפי מודעת המקור — 05.09.2026

- [x] לשמור את הצבעים, התמונה והמסרים של ארבעת הקריאייטיבים הסופיים ללא שינוי
- [x] לשחזר את היררכיית המקור: שורת פתיחה קטנה, כותרת צרה גדולה מאוד, שורת חיזוק ושורת הסבר
- [x] להשתמש בפונט כותרות צר בסגנון Ambivalenti Tzar וב־Rubik לטקסט המשני
- [x] להגדיל את הכפתור ולהחזיר מסגרת שמנת־זהובה בדומה למקור
- [x] להתאים את אותה פריסה לשתי מודעות הפיד ולשתי מודעות הסטורי
- [x] לבדוק אחידות פונט, שבירת שורות, אזורי בטיחות וקריאות בכל ארבע הגרסאות
- [x] למסור את הגרסאות המתוקנות בלי לשנות את האתר או להפעיל קמפיין

## תיקון tRPC Failed to fetch בעמוד הבית — 05.09.2026

- [x] לאתר בלוגי הרשת את `auth.me` ואת `/api/track` שנכשלו כמעט בו־זמנית בזמן האירוע
- [x] לאשר שלא נרשמה שגיאת יישום בשרת וששירות האנליטיקה החיצוני השיב, באופן שתואם ניתוק זמני מהשרת המקומי
- [x] להוסיף עד שני ניסיונות retry מדורגים רק לכשלי רשת זמניים בכל שאילתות tRPC ובפרט `auth.me`, בלי retry לשגיאות הרשאה או mutations
- [x] להוסיף ארבע בדיקות Vitest לזיהוי כשלי רשת, מגבלת retry, השהיה ואכיפת המדיניות ב־QueryClient וב־useAuth
- [x] לבדוק את עמוד הבית בדסקטופ ובמובייל עם `from_webdev=1` ולוודא שאין שגיאת console
- [x] להריץ 83 קובצי בדיקה ו־370 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקה חיה אחת נשארה opt-in
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## תיקון דוח חצות ושדרוג יעדים ותקציבים — 05.09.2026

- [x] לוודא שב־00:00 לפי שעון ישראל הדוח מסכם תמיד את היום המקומי שזה עתה הסתיים, גם אם ה־Heartbeat מגיע כמה דקות אחרי חצות
- [x] לבדוק את חלונות ה־UTC, שעון הקיץ ו־reportDate בריצת Heartbeat וב־backfill
- [x] לבדוק את מודל היעדים הקודם ולאשר שהוא השתמש במשקלי יום כלליים בלבד ללא ביצועים היסטוריים
- [x] לשלב יעדי עסק מאושרים עם 60 ימי תשלומי Grow מאומתים ולידים, ובמוצרים ללא מספיק היסטוריה להשתמש במשקלי סופ״ש וחגים
- [x] להציג לכל מוצר מכירות בפועל מול יעד יומי, שבועי, מצטבר וחודשי ואת הפער מהקצב
- [x] להציג תקציב בפועל מול התכנון ליום, למצטבר ולחודש, כולל סטייה בשקלים ובאחוזים למאגר, באנדל, Boost ושאר התוכנית
- [x] להבהיר ש־CPA ו־ROAS מבוססים על רכישות והכנסות Grow מאומתות ולהציג „לא זמין” רק כשמקור המדידה חסר
- [x] להוסיף בדיקות לתאריך הדוח, חצות מאוחרת, DST, ימים חזקים היסטוריים, יעד מצטבר, תוכנית התקציב וסיווג הקמפיינים
- [x] לבדוק תצוגה מקדימה של שלוש הודעות ה־SMS עם נתונים מצרפיים וללא שליחה
- [x] להריץ 83 קובצי בדיקה ו־373 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות; בדיקה חיה אחת נשארה opt-in
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## יעד הכנסות חודשי בדוח החצות — 05.09.2026

- [x] להגדיר יעד הכנסה חודשי כולל של 140,000 ₪ בקוד ובשורת ההגדרות הפעילה במסד התפעולי
- [x] להציג יעד הכנסה מדויק למאגר: 350 רכישות × 299 ₪ = 104,650 ₪
- [x] להציג יעד הכנסה מדויק לבאנדל: 70 רכישות × 399 ₪ = 27,930 ₪
- [x] להציג יעד הכנסה מדויק ל־Boost: 90 רכישות × 19.90 ₪ = 1,791 ₪
- [x] להציג יעד מוצרים מחושב של 134,371 ₪ ופער של 5,629 ₪ ליעד הכולל ממקורות נוספים
- [x] להוסיף בדיקות חישוב ונוסח, ולהריץ 83 קובצי בדיקה ו־374 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות

## תיקון עריכת יעד הכנסה ופישוט הודעת התקציב — 05.09.2026

- [x] להרחיב את מגבלת יעד ההכנסה בשרת כך ש־140,000 ₪ יישמר ללא שגיאת validation
- [x] לעדכן את מגבלת השדה בממשק ולאפשר עריכה עצמית עד 1,000,000 ₪ עם הודעת שגיאה ידידותית
- [x] לפשט את הודעה 2 כך שכל קמפיין מציג הוצאה היום, הוצאה מצטברת, תכנון מצטבר וסטייה במילים ברורות
- [x] להציג את CTR, CPL, CPA ו־ROAS בשורות קצרות ובשפה יומיומית: אחוז הקלקה, עלות לליד, עלות לרכישה והחזר לכל ₪1
- [x] להוסיף בדיקות עריכת יעד ונוסח, ולהריץ 84 קובצי בדיקה ו־376 בדיקות שעברו, TypeScript, build, git diff --check וסריקת פרטיות

## דוח חצות עדיין לא התקבל ופישוט סוף הודעה 1 — 06.09.2026

- [x] לבדוק את משימת Heartbeat הפעילה ואת שתי ריצות ה־UTC סביב חצות ישראל
- [x] לבדוק daily_report_runs, לוגי הייצור וסטטוסי Vibrate עבור היום שהסתיים
- [x] לאמת ששני הנמענים המאושרים עדיין קיימים ושההגדרה פעילה, בלי לחשוף מספרים
- [x] לתקן דילוג, נעילה או כשל מסירה בלי ליצור שליחה כפולה
- [x] להחליף את שורת יעד המוצרים והפער במשפטים ברורים בעברית
- [x] להחליף את שורת „בסיס קצב” בהסבר קצר שאינו דורש ידע טכני
- [x] להוסיף בדיקות רגרסיה לריצת חצות ולנוסח הודעה 1
- [x] לאמת ריצה יבשה ללא SMS ולא לשלוח השלמה ידנית ללא אישור מפורש
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff --check וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## דוח חצות לא התקבל — 06.09.2026

- [x] לבדוק אם משימת Heartbeat הפעילה רצה בחלונות UTC של חצות ישראל
- [x] לבדוק את לוגי הייצור ואת רשומת daily_report_runs עבור היום שהסתיים
- [x] לבדוק אם הריצה דולגה בגלל נעילת idempotency, תאריך שגוי, הגדרה כבויה או כשל Vibrate
- [x] לתקן את שורש התקלה בלי ליצור שליחה כפולה
- [x] להוסיף בדיקת רגרסיה לריצת חצות ולמסירת שלוש הודעות לשני הנמענים המאושרים
- [x] לאמת ריצה יבשה ללא SMS ולא לשלוח השלמה ידנית ללא אישור מפורש
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff --check וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## הכנת קמפיין המלצות להתאמות מוצלחות ולמסיימי DNA — 06.09.2026

- [x] למפות את מערכת ההמלצות הקיימת, הטופס, המתנה, מסעות ההמשך ומסכי ה־CRM
- [x] להגדיר קהל התאמות מוצלחות לפי מקור אמת היסטורי וללא ספירה כפולה של שני בני הזוג
- [x] להגדיר קהל מסיימי שאלון DNA לפי השלמה מלאה ומתועדת
- [x] להחריג מוסרי דיוור, פרופילים לא פעילים, כתובות חסומות, נמענים ללא הסכמה וכפילויות בין הקהלים
- [x] למנוע פנייה חוזרת למי שכבר מילאו המלצה או שכבר קיימת עבורם בקשה פעילה
- [x] להכין נוסח אישי נפרד לכל קהל, עם הסבר עדין כיצד ההמלצה מגדילה הזדמנויות במאגר
- [x] ליצור קישור אישי מאובטח לטופס ולתעד מקור קהל, מוצר ושלב בתהליך
- [x] להכין תור טיוטה בלבד ללא שליחת מייל, SMS או WhatsApp
- [x] להציג ב־CRM את כמות הזכאים, המוחרגים, הכפילויות והטיוטות לפי קהל
- [x] להוסיף מנגנון אישור מפורש ונפרד לפני כל שליחה חיצונית
- [x] להוסיף בדיקות פרטיות, זכאות, מניעת כפילויות, הסרה ושמירת מצב טיוטה
- [x] להפיק ריצה יבשה עם כמויות ונוסחים לדוגמה ללא פרטי לקוחות וללא שליחה
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff --check וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## השוואת קצב לידים, תקציבים ומכירות — 06.09.2026

- [x] להגדיר גבולות זמן זהים לפי שעון ישראל ל־25.7–5.8 ול־25.8–5.9
- [x] לזהות את מקור האמת להוצאות Meta ולסווג רק קמפיינים מכירתיים רלוונטיים
- [x] לספור לידים חדשים בשתי התקופות לפי אותה הגדרה וללא כפילויות
- [x] לספור יחידות מאגר וט״ו באב שנמכרו לפי עסקאות Grow מאומתות בלבד, או אומדן היסטורי מסומן כאשר Grow אינו זמין
- [x] לחשב הכנסות מאגר וט״ו באב בנפרד וביחד בכל תקופה
- [x] לחשב קצב יומי, CPL, CPA ו־ROAS על בסיס הנתונים הזמינים
- [x] לפרק את התקופה האחרונה לפי יום ולזהות שינוי בימים האחרונים
- [x] לאמת את המספרים מול מקורות התפעול ולהציג פערי כיסוי או מגבלות נתונים
- [x] להציג השוואה ומסקנות עסקיות קצרות וברורות

## דיוק מסעות ההמלצות למאץ׳ הדדי ול־DNA — 06.09.2026

- [x] למפות כל אדם שהיה במאץ׳ שאושר על ידי שני הצדדים ואת מצב בקשת ההמלצה שלו
- [x] להפריד בין מי שטרם נתנו עדות לבין מי שכבר נתנו עדות בעבר
- [x] להכין קהל פנייה ראשונה לכל מאץ׳ הדדי שלא נתן עדות, בכפוף להסכמה, פעילות ופרטי קשר תקינים
- [x] להכין קהל פנייה חוזרת לממליצים קיימים כדי לבדוק אם הם עדיין יחד ולהציע שיתוף תמונה וסיפור הצלחה
- [x] לוודא ששיתוף תמונה, סיפור ופרסום יישארו אופציונליים וידרשו הסכמה מפורשת
- [x] לצמצם את קהל ה־DNA למסיימי השאלון והמסע שלא רכשו אך פתחו את המיילים
- [x] למנוע חפיפה, פנייה כפולה והכללת מוסרי דיוור, חסומים או פרופילים לא פעילים
- [x] לנסח פנייה נפרדת למאץ׳ הדדי, לממליצים קיימים ולמסיימי DNA
- [x] להציג ב־CRM את הכמויות, ההחרגות והנוסחים של שלושת הקהלים
- [x] להכין טיוטות בלבד ללא תזמון וללא שליחת מייל, SMS או WhatsApp
- [x] להוסיף בדיקות רגרסיה לקהלים, לנעילת השליחה ולהסכמת תמונה ופרסום
- [x] להריץ ריצה יבשה ובדיקות מלאות, לפרסם ולסנכרן ל־GitHub
- [x] להציג לאישור מפורש את הכמויות והנוסחים לפני כל שליחה

## שליחת קמפיין ההמלצות ועדכון סוף יום — 06.09.2026

- [x] לקבע את אישור המשתמשת לשליחת 216 פניות מאץ׳, 33 מעקבי הצלחה ו־100 פניות DNA בלבד
- [x] לוודא לפני השליחה שכל 349 הרשומות עדיין טיוטות, ללא תזמון או שליחה קודמת
- [x] לאשר את שלושת הקהלים למסירה במנות מבוקרות ולשמור idempotency
- [x] לשלוח את 349 בקשות ההמלצה דרך ערוץ המייל הקיים בלבד
- [x] לאמת את מספר ההצלחות, הכשלים וההחרגות ללא חשיפת פרטי נמענים
- [x] לוודא שלא נוצרו כפילויות ושלא נשלחו SMS או WhatsApp
- [x] לתזמן עדכון להילית בשעה 23:45 לפי שעון ישראל עם פתיחות ותגובות בפועל
- [x] לתעד בעדכון סוף היום את בסיס המדידה ואת מגבלות מעקב הפתיחות
- [x] להריץ בדיקות רגרסיה, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם את שינויי הבקרה ולסנכרן את אותו checkpoint ל־GitHub

## הסרת פרופיל וחסימת דיוור מלאה — 06.09.2026

- [x] לאתר ולאמת את הפרופיל המדויק לפי הפרטים שצורפו
- [x] למפות התאמות פעילות, טיוטות מייל ומסעות עתידיים בלי לחשוף פרטים אישיים
- [x] להסיר את הפרופיל מהמאגר הפעיל ומכל מסלול Boost
- [x] לשחרר התאמות פעילות ולהחזיר את הצד השני למאגר כאשר נדרש
- [x] לבטל כל מייל ממתין או טיוטת פנייה עתידית
- [x] להחיל הסרת דיוור וחסימה שיווקית בכל מקורות האמת ובספק המייל
- [x] לאמת שהפרופיל אינו פעיל, שאין התאמה פעילה ושאין הודעה עתידית
- [x] לעדכן את מעקב המשימה, לשמור checkpoint ולסנכרן ל־GitHub

## תקיעה לפני מסך התשלום — עוז אלהרר — 06.09.2026

- [x] לאתר את הרשומה וניסיון התשלום של עוז לפי המייל שסופק
- [x] לזהות לאיזה מוצר ועמוד תשלום ניסה להגיע ובאיזה שלב נעצר
- [x] לבדוק לוגי שרת, דפדפן ורשת בלי לחשוף פרטים אישיים
- [x] לשחזר את התקלה ללא הזנת כרטיס וללא חיוב
- [x] לתקן את גורם התקיעה בלי לפגוע במסלולי תשלום אחרים
- [x] להוסיף בדיקת רגרסיה למסלול שנכשל
- [x] לאמת שהמעבר למסך התשלום עובד ללא ביצוע עסקה
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## ניסוח מסלול ההצטרפות בלשון זכר — עוז אלהרר — 06.09.2026

- [x] לאתר את כל הניסוחים הקבועים בלשון נקבה במסלול `/join`
- [x] להתאים את הכותרות, ההנחיות והודעות הפעולה למגדר מהקישור
- [x] לשמור ניסוחים טבעיים וללא צורות לוכסן מיותרות
- [x] להוסיף בדיקות רגרסיה לקישור `gender=male`
- [x] לאמת חזותית במובייל שהעמוד מוצג בלשון זכר
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## התאמת הכנסות הדאשבורד ל־Grow — 01.09–06.09.2026

- [x] להגדיר במדויק אילו עסקאות, זיכויים ומקורות נכללים בדאשבורד וב־Grow
- [x] לשלוף לכל יום את הכנסות הדאשבורד ואת עסקאות Grow המאומתות
- [x] להצליב יחידות, סכומים, מוצרים, זיכויים, עסקאות בדיקה וכפילויות
- [x] לזהות מדוע הכנסות הדאשבורד גבוהות יותר בכל ימי השבוע
- [x] לתקן את החישוב כך שהמדד הראשי יתבסס על מקור האמת הנכון
- [x] להוסיף פירוט ברוטו, זיכויים ונטו אם נדרש כדי למנוע בלבול חוזר
- [x] להוסיף בדיקות רגרסיה לכללי הספירה ולגבולות יום ישראל
- [x] לאמת יום־יום מול נתוני Grow מ־1.9 ועד 6.9
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## ניתוח ניסיונות תשלום שלא הושלמו — 01.09–06.09.2026

- [x] להגדיר מהו ניסיון תשלום אמיתי לעומת פתיחת טופס או רשומה כפולה
- [x] לשלוף את כל ניסיונות התשלום בתקופה ולקשר אותם לעסקאות Grow שהושלמו מאוחר יותר
- [x] לבדוק לכל ניסיון מוצר, זמן, שלב אחרון ואירועי שגיאה זמינים
- [x] לזהות כפילויות, ניסיונות בדיקה, פרופילים חסומים ומי שכבר רכשו
- [x] לסווג כל מקרה כנטישה רגילה, כשל טכני אפשרי, רכישה מאוחרת או רשומה כפולה
- [x] לאמת אם קיימת תקלה מערכתית משותפת במסלול התשלום
- [x] להכין רשימת טיפול מאובטחת ב־CRM ללא חשיפת פרטים אישיים בצ׳אט
- [x] להציע שתי חלופות פנייה ולנסח הודעה מתאימה לכל סוג מקרה
- [x] להציג כמויות ומסקנות לאישור מפורש לפני כל מייל, SMS או WhatsApp

## חידוד הטבת החג בעמוד new-year-love — 07.09.2026

- [x] לבדוק את תוכן העמוד החי ואת המחירים והפלואו הקיימים
- [x] לאמת את המחיר הרגיל והמחיר המיוחד של המאגר, הקורס והמדריך
- [x] לבחור מבנה הטבה חד: מאגר במרכז ושני המוצרים הנוספים כלולים
- [x] לנסח כותרת והסבר שמבהירים מיד מה מקבלים ומה החיסכון
- [x] להחזיר לעמוד הסבר ברור ונפרד על המאגר, הקורס והמדריך
- [x] לשמור על המסר „להיכנס אחרת לחגים ולצאת מהם אחרת” בלי הבטחה מוגזמת
- [x] לעדכן את העמוד בלי לשנות את פלואו התשלום או המוצר שנמסר
- [x] להוסיף או לעדכן בדיקות רגרסיה למחיר, המוצרים וה־CTA
- [x] לאמת חזותית בדסקטופ ובמובייל ולבדוק את הקישור לתשלום ללא חיוב
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## פישוט מחדש של הטבת החג — 07.09.2026

- [x] לאמת את השווי המקורי של המאגר, המדריך והקורס ואת מחיר החבילה 399 ₪
- [x] להסיר מהעמוד פירוט מחירים וחישובי ביניים שחוזרים על עצמם
- [x] להשאיר מסר אחד: המאגר הוא הלב והמדריך והקורס עוזרים להגיע מוכנים
- [x] להציג רק את השווי המקורי הכולל מול 399 ₪ ואת סך ההנחה
- [x] להחזיר הסבר קצר על כל מוצר ולמה שלושתם משלימים זה את זה
- [x] להוסיף מסר שזו הטבה שלא הייתה בעבר ושכעת הילית שמה גז על ההתאמות
- [x] להפוך את שעון הספירה לאחור לבולט ודחוף יותר בלי הבטחה מטעה
- [x] לשמור את מוצר התשלום, מחיר החיוב ופלואו המסירה ללא שינוי
- [x] לעדכן בדיקות ולהריץ QA בדסקטופ ובמובייל ללא חיוב
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## מודעת פיד לחבילת החג — 07.09.2026

- [x] לאתר את תמונת הילית המתאימה ואת נכסי הנראות החומה־קרמית
- [x] להגדיר מסר קצר והיררכיה לפורמט פיד 1:1
- [x] ליצור מודעה עם המאגר במרכז, המדריך והקורס כהכנה משלימה
- [x] להציג 399 ₪ במקום 1,245 ₪ בלי עומס טקסט
- [x] לשלב פרחים לבנים עדינים, צבעוניות חגיגית וזהות המותג
- [x] לבדוק עברית, מחיר, קריאות ואזורי בטיחות למודעות Meta
- [x] להכין טקסט מלווה, כותרת ותיאור קצרים לקמפיין
- [x] למסור את קובץ המודעה והטקסטים לאישור

## מודעת זוגיות חומה לחבילת החג — 07.09.2026

- [x] לנתח את רפרנס הזוגיות ואת הגרסאות החומות שאושרו
- [x] להגדיר זוג אווירה שאינו מוצג כזוג אמיתי או כעדות
- [x] לבנות מסר קצר על הגעה יחד לחגים והמאגר כלב ההצעה
- [x] להציג את שלושת המוצרים ב־399 ₪ במקום 1,245 ₪
- [x] ליצור גרסת פיד 1:1 בנראות חומה־קרמית עם פרחים לבנים
- [x] לבדוק עברית, מחיר, קריאות, אזורי בטיחות ואמינות המסר
- [x] להכין טקסט מלווה תואם ולהציג את המודעה לאישור

## מודעת זוגיות עם התמונה האמיתית — 07.09.2026

- [x] להשתמש בתמונת הזוג האמיתית שצורפה כרפרנס הראשי
- [x] לשמר את הפנים, הגוף והזהות של שני המצולמים ללא החלפה
- [x] להחליף את רקע התמונה לנראות חומה־חגיגית עם פרחים לבנים
- [x] להשתמש בפונט כותרת צר ובפונט Rubik לטקסט המשלים
- [x] לשלב את מסר ההגעה יחד לשולחן החג ואת ההטבה 399 ₪ במקום 1,245 ₪
- [x] לבדוק שימור פנים, עברית, מחיר, קריאות ואזורי בטיחות
- [x] למסור את גרסת הפיד עם התמונה האמיתית לאישור

## גרסת טיזר לזוגיות ללא מחיר — 07.09.2026

- [x] לשמור את התמונה האמיתית ואת הרקע החום של הגרסה שאושרה
- [x] להסיר את פירוט המוצרים, המחיר והשווי המקורי
- [x] להשאיר את המסר „אולי השנה תגיעו יחד לשולחן החג”
- [x] להוסיף רק „הטבת חג מיוחדת מחכה לכם” וכפתור קצר
- [x] לבדוק עברית, קריאות, שימור פנים ואזורי בטיחות
- [x] למסור את גרסת הטיזר לצד גרסת ההטבה המלאה

## הארכת קוד HOLIDAY10 — 07.09.2026

- [x] להאריך את תוקף HOLIDAY10 עד 30.9.2026 בשעה 23:59 לפי שעון ישראל
- [x] לשמור על 10% הנחה לחבילת החג בלבד וללא הגבלת שימושים
- [x] לאמת שהקוד פעיל ואינו פג תוקף לאחר העדכון
- [x] לתעד את השינוי בלי לבצע רכישה או חיוב

## תיקון HOLIDAY10 שמוצג כלא בתוקף — 07.09.2026

- [x] לשחזר את הודעת חוסר התוקף דרך מסלול הקופון שבו משתמש עמוד התשלום
- [x] לבדוק את מסד התפעול של האתר החי ואת מקור הנתונים המקומי
- [x] לאתר תנאי תוקף, מטמון או חיבור מסד שעדיין משתמשים בתאריך הישן
- [x] לתקן את מקור התקלה בלי לשנות את 10% ההנחה או תחולת חבילת החג
- [x] להוסיף בדיקת רגרסיה לתוקף הקופון עד סוף ספטמבר
- [x] לאמת שהקוד מתקבל באתר החי ומציג את המחיר המוזל ללא חיוב
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## מעבר הוראת הקבע של Database Plus ל־Grow Live — 07.09.2026

- [x] לבצע שינוי מינימלי בלבד: כתובות secure ומזהי Production במסלול Plus
- [x] להשאיר ללא שינוי את chargeType, הסכום, הפלואו והחסימה הציבורית הקיימת
- [x] לבצע אימות מקומי בלבד עם mock, ללא קריאת Live נוספת, כרטיס או חיוב
- [x] למפות את כל קריאות יצירת התשלום ואישור העסקה במסלול Plus
- [x] לוודא שכל כתובות Sandbox מוחלפות ב־secure במסלול ה־Live
- [x] לשמור את userId ו־pageCode של Live כסודות שרת בלבד
- [x] לוודא שמסלול Plus נשאר הוראת קבע ולא הופך לעסקה רגילה
- [x] להסיר fallback למזהי Sandbox או למסלולי תשלום אחרים
- [x] להוסיף בדיקות רגרסיה לכתובות secure, למזהי Live ולסוג החיוב
- [x] לאמת מקומית את יצירת בקשת ה־Live ללא הזנת כרטיס וללא ביצוע חיוב
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## פתיחת Database Plus לציבור — 07.09.2026

- [x] לפתוח את `/database-plus`, `/terms/plus` ו־`/thank-you/plus` לציבור
- [x] להסיר את חסימת יצירת תשלום Plus ב־API
- [x] להציג תשלום חודשי מתחדש של 99 ₪ דרך Grow Production
- [x] לשמור את שלוש הסכמות החובה לפני יצירת התשלום
- [x] לוודא שהעמוד אינו דורש חברות קיימת במאגר או קישור אישי
- [x] לבדוק את הכנת מעבר ה־Hosted URL ל־Grow ללא הזנת כרטיס וללא חיוב
- [x] להוסיף כיסוי רגרסיה למחיר, recurring, הסכמות והנתיבים הציבוריים
- [x] להריץ בדיקות מלאות, TypeScript, build, git diff וסריקת פרטיות
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## עמוד סיום, זכויות Plus ופיילוט 20 נשים ו־20 גברים — 07.09.2026

- [x] להסיר מעמוד התודה ניסוחים טכניים על Grow ותהליך הסליקה
- [x] להסביר בעמוד התודה שמגיעות שתי התאמות במסלול Plus ובוסט אחד חינם
- [x] לאמת ששתי ההתאמות מוגדרות במוצר ובהסכמות בצורה עקבית
- [x] לוודא שבוסט חינם אחד מוקצה אוטומטית רק אחרי תשלום Plus מאושר
- [x] להוסיף בדיקות רגרסיה להקצאת הבוסט ולמניעת הקצאה לפני תשלום
- [x] לבחור 20 נשים ו־15 גברים חדשים, ולצרף להזדמנות עוד 5 גברים היסטוריים שחויבו 1 ₪ בלבד ולא היו מנויי Plus אמיתיים
- [x] להחריג מוסרי דיוור, חסומים, כפילויות ומנויי Plus קיימים
- [x] ליצור 35 הזמנות אישיות לפיילוט ללא הפעלת מנוי לפני תשלום
- [x] לנסח מייל חג שמסביר את שתי ההתאמות, הבוסט החינמי והתשלום של 99 ₪ לחודש
- [x] לאמת את קישור התשלום וההסרות בכל ההזמנות; היעד הורחב בהמשך באישור מפורש ל־60
- [x] לשלוח את הקמפיין המאושר; משימת 35 הנמענים הוחלפה ואושרה כקהל של 30 נשים ו־30 גברים
- [x] לאתר 5 גברים שניסו Plus ב־Sandbox וחויבו 1 ₪ בלבד, ולהפריד אותם ממנויי Plus אמיתיים בלי לפגוע בחברות Boost עצמאית
- [x] להכין נוסח WhatsApp עם קישור חיוב אישי, ללא שליחת WhatsApp אוטומטית
- [x] לשלוח השלמת תשלום רק למי שאומת שלא חויב ולא פעיל: 3 מורשים קיבלו מייל ו־SMS, ו־2 מוסרי דיוור הוחרגו
- [x] לאמת חזותית את עמוד התודה ואת חוויית Plus במובייל ובדסקטופ
- [x] להריץ 93 קובצי בדיקה ו־412 בדיקות, TypeScript, build, git diff וסריקת פרטיות; 92 קבצים ו־411 בדיקות עברו ואחד דולג כמתוכנן
- [x] לפרסם ולסנכרן את אותו checkpoint ל־GitHub

## תצוגת וטסט למייל הזמנת Database Plus — 07.09.2026

- [x] ליצור תצוגה זהה למייל ההזמנה של Plus ולצלם אותה כתמונה למסירה
- [x] לשלוח מייל טסט יחיד בלבד לכתובת שאושרה, בלי לצרף אותה לקמפיין הפיילוט
- [x] לוודא שכפתור המייל מוביל לעמוד Plus הציבורי התקין; כתובת הטסט אינה פרופיל מאגר ולכן לא נוסף לה טוקן אישי
- [x] לוודא שניתן ליצור מעבר ל־Grow Production בסך 99 ₪ בחיוב חודשי, ללא הזנת כרטיס וללא ביצוע עסקה
- [x] לתעד את סטטוס השליחה והבדיקה בלי לחשוף קישור אישי, טוקן או מזהי תשלום

## שדרוג מייל ההזמנה החגיגי ל־Database Plus — 07.09.2026

- [x] לפתוח במסר אישי: ראינו שנרשמת לרשימת ההמתנה ושמחים לבשר שנבחרת להשקה
- [x] להציג את Database Plus כשירות ההיכרויות המתקדם ביותר של הילית וכהזדמנות מיוחדת לחברי המאגר
- [x] להפריד חזותית בין שתי התאמות בכל חודש, בוסט אחד חינם בכל מחזור ותשומת הלב המוגברת לפרופיל
- [x] לשלב מסר חגיגי של השקה ושל הילית ששמה גז על ההיכרויות לחברי המאגר
- [x] לשמור על הבטחות מדויקות, חיוב חודשי של 99 ₪, קישור אישי והפעלה רק אחרי תשלום מאושר
- [x] לעדכן בדיקות תוכן, להפיק תצוגה חזותית חדשה ולאמת קישור לפני פרסום
- [x] לפרסם ולסנכרן ל־GitHub את המייל המעודכן לפני שליחת קמפיין הפיילוט

## הרחבת פיילוט Plus ל־30 נשים ו־30 גברים וטסט SMS — 07.09.2026

- [x] להגדיר מקור אמין לזיהוי כל לקוחות הליווי ולהחריג אותם מכל קהל Plus
- [x] לוודא ששני השמות שהוחרגו במפורש אינם נכללים בקהל גם אם לא סומנו קודם כלקוחות ליווי
- [x] לחשב קהל כשיר של 30 נשים ו־30 גברים תוך שמירת כל תנאי הכשירות, ההסכמה וההסרה
- [x] לא לשנות או לשלוח לקהל ה־60 לפני הצגת רשימה מוגנת ואישור סופי
- [x] להציג רשימת קהל מוגנת ללא אימיילים, טלפונים, טוקנים, מזהים פנימיים או קישורי תשלום
- [x] לנסח SMS חגיגי וקצר שמודיע שנבחרו ומפנה לקישור ההצטרפות האישי
- [x] לשלוח SMS טסט אחד בלבד למספר שאושר ולאמת קבלת בקשת השליחה אצל ספק ה־SMS
- [x] להריץ בדיקות, לפרסם ולסנכרן ל־GitHub רק אם נדרש שינוי קוד קבוע

## שליחת פיילוט Plus במייל וב־SMS ומסלול השלמת תשלום — 07.09.2026

- [x] לעדכן את SMS ההשקה לפתיחה „היי, כאן הילית”, רווחים ואימוג׳ים עדינים
- [x] להוסיף לכל SMS קישור אישי ל־Plus, תיעוד ספק ומניעת שליחה כפולה
- [x] לנסח SMS נפרד ושקוף למקרי 1 ₪ שמבהיר שלא הוגדר מנוי 99 ₪ ומאפשר השלמת תשלום
- [x] לוודא שמקרי הסרה מדיוור אינם מקבלים מייל או SMS גם בקבוצת ההתאוששות
- [x] לאמת שהפעלת מנוי Plus והבוסט מתבצעת רק לאחר תשלום Production מאושר של 99 ₪
- [x] להריץ שערי שחרור מלאים, לפרסם ולסנכרן ל־GitHub לפני השליחה
- [x] לבצע preflight סופי של 30 נשים, 30 גברים ו־3 מקרי השלמה מורשים
- [x] לשלוח 60 מיילי השקה ו־60 הודעות SMS אישיות ללא כפילויות
- [x] לשלוח ל־3 מקרי ההשלמה המורשים מייל ו־SMS, ללא פנייה ל־2 מוסרי הדיוור
- [x] לאמת מצרפית קבלה אצל ספקי הדיוור וה־SMS ואפס הפעלות מנוי לפני תשלום

## ניהול Plus, תצוגה חודשית ושליחת בקשות המלצה — 07.09.2026

- [x] לחשב במדויק כמה התאמות ייחודיות נשלחות השבוע, כאשר שני אנשים נחשבים התאמה אחת
- [x] להוסיף לדאשבורד ייעודי של Plus רשימת מנויים שהופעלו רק לאחר תשלום מאושר
- [x] להציג לכל מנוי Plus את מספר ופרטי ההתאמות שנשלחו בחודש הנוכחי
- [x] להציג לכל מנוי Plus אם הבוסט החינמי של המחזור נוצל או עדיין זמין
- [x] להגדיר בדאשבורד הכנסות והוצאות תצוגת חודש נוכחי מתחילתו כברירת מחדל
- [x] להשאיר אפשרות לבחור טווחי זמן אחרים בדאשבורד הכנסות והוצאות
- [x] להוסיף במערכת משובים והמלצות פעולה לשליחת מיילים מטיוטות מוכנות
- [x] להגן על שליחת בקשות המלצה באמצעות בדיקת הסכמה, הסרה, כפילות וסטטוס טיוטה
- [x] להוסיף בדיקות Vitest לכל שינויי השרת והממשק
- [x] לבצע אימות חזותי בדסקטופ ובמובייל למסך ההמלצות, ואימות runtime ו־build למסכי Plus והכנסות המוגנים
- [x] לפרסם ולסנכרן את הגרסה הסופית ל־GitHub

## ניתוח קמפיינים ותכנון קמפיין סיפורי הצלחה — 07.09.2026

- [x] לאסוף נתוני תקציב, לידים, רכישות, הכנסות, CPL, CPA ו־ROAS מכל הקמפיינים הפעילים והרלוונטיים
- [x] להפריד בין קמפייני שאלון DNA, מאגר, חבילות, Boost ויתר המוצרים
- [x] לבדוק את איכות הלידים מהשאלון לפי מעבר למאגר ורכישה בפועל, ולא רק לפי עלות ליד
- [x] להשוות מטרת Leads מול Sales לקהל קר, קהל חם וריטרגטינג
- [x] לזהות אילו מסרי סיפורי הצלחה מתאימים לשלב העליון, האמצעי והתחתון של המשפך
- [x] להמליץ על מבנה קמפיינים נפרד או משולב, קהלים, תקציב ניסוי ומדדי עצירה/הגדלה
- [x] למסור תמצית ברורה שניתן להעביר למנהל הקמפיינים

## תקלה בקוד הצטרפות חינמית למאגר — 08.09.2026

- [x] לאתר את רשומת המשתמש וקוד ההטבה שסופק בלי לחשוף אותם בלוגים או בדיווח
- [x] לבדוק אם הקוד קיים, פעיל, בתוקף, מוגבל למוצר הנכון או כבר נוצל
- [x] לבדוק את נרמול הקוד ואת בקשת האימות שנשלחת מעמוד `/join`
- [x] לתקן את מקור הכשל באופן ממוקד בלי לבצע חיוב או להצטרף בשם המשתמש
- [x] להוסיף בדיקות רגרסיה לקוד חינמי, קוד לא תקין וקוד שכבר נוצל
- [x] לאמת באתר החי שהקוד מתקבל כגישה חינמית ללא צורך בתשלום, ללא פדיון, הרשמה או חיוב
- [x] לפרסם ולסנכרן ל־GitHub את התיקון

## התאמה שנחסמה בגלל סטטוס פעיל אחר — 08.09.2026

- [x] לזהות את שתי רשומות המשתמשים מהצילום ואת כל ההתאמות הפעילות וההיסטוריות שלהם
- [x] לבדוק אם ההתאמה המבוקשת נוצרה, נכשלה, נמחקה או נשארה בטיוטה
- [x] לזהות שההתאמה הפעילה שמופיעה לשני הצדדים היא ההתאמה המבוקשת ביניהם, ולא התאמה אחרת
- [x] לאמת שלא קיימים סטטוסים שגויים לשחרור: ההתאמה הפעילה היא הזוג המבוקש ולכן לא שונתה התאמה אחרת
- [x] ליצור או להחזיר את ההתאמה המבוקשת בין שני הצדדים ולוודא שהיא נשמרה כ־proposed ב־CRM
- [x] לשלוח לשני הצדדים את מיילי ההתאמה המאושרים ללא כפילות
- [x] לאמת מצרפית ששני המיילים התקבלו אצל ספק הדיוור ושההתאמה הפעילה היא הנכונה
- [x] להוסיף בדיקת רגרסיה לרענון טאב „קיבלו התאמה” לאחר שליחה ישירה ממסך בדיקת ההתאמה
- [x] להריץ 96 קובצי בדיקה ו־429 בדיקות, TypeScript, build ו־diff; 95 קבצים ו־428 בדיקות עברו ואחד דולג כמתוכנן
- [x] לפרסם ולסנכרן ל־GitHub את תיקון רענון ההתאמה ב־CRM

## משתמשת שאינה מופיעה במאגר — 08.09.2026

- [x] לחפש את הרשומה לפי אימייל, שם ווריאציות כתיב בכל טבלאות המשתמשים, הלידים והשאלונים
- [x] לבדוק אם קיים תשלום מאומת למאגר ומהו המוצר שנרכש; לא נמצא תשלום תחת הכתובת שסופקה
- [x] לבדוק אם הפרופיל קיים אך אינו פעיל, הוסר, חסר פרטים או מסומן כטיוטה; לא נמצאה רשומת פרופיל
- [x] לבדוק אם קיימת הסרת דיוור או בקשת מחיקה שצריכה למנוע החזרה אוטומטית; לא נמצאה רשומת קשר או הסרה תחת הכתובת
- [x] לקשר ולשחזר את הרשומה לאחר שנמצאו עסקת 299 ₪ מאומתת, פרופיל קיים וללא בקשת הסרה
- [x] לאמת שהמשתמשת פעילה ומופיעה בחיפוש המאגר לאחר התיקון, בלי לחשוף פרטים אישיים בדיווח

## עסקת Grow שלא קושרה לפרופיל המאגר — 08.09.2026

- [x] לחפש את העסקה המאומתת לפי שם, מוצר, סכום ותאריך גם כאשר ב־Grow הופיע אימייל שונה
- [x] להתאים את עסקת Grow לרשומות webhook, ליד התשלום, ה־CRM והפרופיל הקיימים
- [x] לוודא שאין פרופיל קיים נוסף תחת אימייל או טלפון חלופיים לפני השחזור
- [x] לשחזר ולקשר את פרופיל המאגר רק על בסיס עסקה מאומתת ופרטי הרשמה תואמים
- [x] לא לבצע חיוב נוסף ולא לשלוח מייל כפול במהלך השחזור
- [x] לאמת שהפרופיל פעיל, משויך לתשלום ומופיע בחיפוש המאגר
- [x] לקבוע שלא נדרש תיקון קוד: ה־webhook והפרופיל נמצאו במסד התפעולי, והכשל היה סטטוס פרופיל לא פעיל
- [x] לשמור checkpoint ולסנכרן ל־GitHub את תיעוד השחזור

## שחזור מלא והפעלת רחל במאגר — 08.09.2026

- [x] לחפש את כל הנתונים לפי האימייל הנכון, האימייל השגוי שבעסקת Grow, הטלפון, השם ואסמכתת העסקה
- [x] לאתר ולחבר תוצאת DNA, תמונה, פרטי פרופיל והיסטוריית CRM; לא נמצאו תשובות לשאלון ההתאמה ולא הומצאו תשובות חסרות
- [x] לוודא שאין פרופיל כפול פעיל או מוסר דיוור לפני איחוד הרשומות
- [x] לתעד את עסקת 299 ₪ המאומתת מ־Grow תחת האימייל הנכון בלי לבצע חיוב נוסף
- [x] לשחזר את הפרופיל הקיים עם נתוני המגדר והפרטים המאומתים, בלי ליצור פרופיל חלקי או להמציא מידע
- [x] להפעיל את הפרופיל במאגר ולקשר את התשלום, ה־CRM ותוצאת ה־DNA לרשומה אחת
- [x] לאמת שהפרופיל מופיע בחיפוש, פעיל להתאמות ולא נוצרה כפילות
- [x] לקבוע שלא נדרש שינוי קוד נוסף: זה היה תיקון נתון תפעולי נקודתי
- [x] לשמור checkpoint ולסנכרן ל־GitHub את תיעוד השחזור

## תקלה חוזרת בטוקן גישה חינמית בעמוד `/join` — 08.09.2026

- [x] לבדוק מחדש את הטוקן המדויק והאימייל במסד התפעולי; הטוקן קיים, תואם, בתוקף ולא נוצל
- [x] לשחזר באתר החי הדבקה של הטוקן המלא בדיוק כפי שהמשתמש מבצע אותה
- [x] לבדוק אם שדה הקופון חותך או משנה את הטוקן; השדה הציג את סוף הטוקן בגלל גלילה אופקית אך שלח את המחרוזת המלאה
- [x] לבדוק אם עמוד `/join` שולח את הטוקן ל־coupon.validate במקום ל־invites.validate; זה היה מקור הכשל
- [x] לתקן את המסלול כך שטוקן גישה חינמית יזוהה גם בשדה שבו המשתמש הזין אותו וגם בקישור `free_token` שה־CRM יוצר
- [x] להציג במסלול החינמי „אימות” ו„המשך להרשמה החינמית” במקום מחיר או ניסוח תשלום
- [x] להוסיף בדיקות רגרסיה לטוקן מלא ארוך, הדבקה, רישיות, רווחים, קישור CRM והסתרת התשלום לאחר אימות
- [x] להריץ 97 קובצי בדיקה ו־433 בדיקות, TypeScript, build ו־diff; 96 קבצים ו־432 בדיקות עברו ואחד דולג כמתוכנן
- [x] לפרסם ולסנכרן ל־GitHub את התיקון
- [x] לאמת באתר החי שהקישור מציג „כניסה חינמית למאגר אושרה” ו„המשך להרשמה החינמית”, ושגם הדבקת הקוד מאומתת, בלי לפדות אותו ובלי חיוב

## אימות מסירת התאמות וסיכום שבועי — 09.09.2026

- [x] להגדיר את חלון אתמול ואת חלון שבעת הימים האחרונים לפי שעון ישראל
- [x] לאסוף 60 התאמות ייחודיות שנשלחו אתמול, כאשר שני אנשים נחשבים התאמה אחת
- [x] לוודא שלכל התאמה מאתמול קיימים שני מיילים; 59 זוגות נמסרו במלואם ואצל זוג Boost אחד שני המיילים החסרים נשלחו מחדש ונמסרו היום
- [x] לוודא שכל 60 ההתאמות מאתמול מופיעות היום בטאב המעקב המתאים ב־CRM
- [x] לזהות ולתקן את החריגה היחידה: התאמת Boost שנשמרה ב־CRM אך שני מייליה לא התקבלו אצל Brevo
- [x] לתקן את מנגנון ה־retry כך ש־emailRetriedAt יסומן רק כאשר Brevo קיבל את שני מיילי ההתאמה, כדי שכשל חלקי לא ייעלם מהטיפול
- [x] לחשב שבשבעת הימים האחרונים נשלחו 217 התאמות: 20 הצליחו, 45 ממתינות ו־152 שוחררו או לא המשיכו
- [x] להריץ 98 קובצי בדיקה ו־436 בדיקות, TypeScript, build ו־diff; 97 קבצים ו־435 בדיקות עברו ואחד דולג כמתוכנן
- [x] להכין דיווח מצרפי בלבד בלי לחשוף שמות, אימיילים או מזהי התאמה

## תקלה קריטית במסלול תשלום המאגר וטוקן חינמי — 09.09.2026

- [x] לשחזר באתר החי את שמירת הפרופיל ויצירת Grow לאחר הרשמה ישירה ולאחר מעבר משאלון ה־DNA באמצעות פרופילי QA זמניים שנמחקו
- [x] לבדוק בלוגי הייצור ואת ה־API החי; לא נמצא כשל קבוע ב־Grow, ושמירת פרופיל רגיל, פרופיל DNA ותמונה בגודל 4.8MB הצליחו
- [x] לבדוק אם התקלה נגרמה משינויי הטוקן או מפער סכמת מסד; לא נמצא פער סכמתי והכשל התרחש בשמירת הפרופיל לפני הקריאה ל־Grow
- [x] לחזק את שמירת הפרופיל בשלושה ניסיונות אוטומטיים ולדחוס תמונות מובייל לפני ההעלאה, בלי לשנות סכום או תנאי תשלום
- [x] להציג כבר בראש טופס ההצטרפות שדה ברור ל„קוד כניסה חינמית”, בנוסף לשדה שלפני אזור התשלום
- [x] לוודא שטוקן חינמי תקין מסתיר את התשלום ומוביל להרשמה חינמית, וטוקן לא תקין אינו מפעיל פרופיל
- [x] להוסיף בדיקות רגרסיה לכניסה ישירה, המשך מה־DNA, retry, תמונה גדולה, תשלום רגיל וטוקן חינמי
- [x] להריץ 97 קובצי בדיקה ו־435 בדיקות, TypeScript, build ו־diff; 96 קבצים ו־434 בדיקות עברו ואחד דולג כמתוכנן
- [x] לפרסם ולסנכרן ל־GitHub את התיקון לפני אימות בייצור
- [x] לאמת באתר החי ששמירת פרופיל ויצירת Hosted Payment של Grow פועלות במסלול הישיר ובמסלול DNA, ללא הזנת כרטיס וללא חיוב
- [x] לאמת באתר החי ששדה „קוד כניסה חינמית” מופיע בראש הטופס ושמסלול טוקן תקין עובר להרשמה חינמית בלי תשלום

## חקירת אירוע התשלום וניטור אוטומטי — 09.09.2026

- [x] לבנות ציר זמן: עדות ראשונה ב־10:30, checkpoint מתקן ב־10:56 והתייצבות מלאה בייצור ב־11:16 לפי שעון ישראל
- [x] לקבוע חלון השפעה מאומת מרבי של 46 דקות; אין ראיה לכשל לפני 10:30 ולא נמצאה השבתת Grow רציפה
- [x] לכמת את המושפעים במגבלות הנתונים: משתמש אחד מאומת מהצילומים; המספר המדויק אינו ניתן לשחזור כי כשלים לפני Grow לא נשמרו עם מזהה או אימייל
- [x] לזהות שהניטור הקיים התחיל רק ביצירת תהליך Grow או ב־SDK, ולכן כשל שמירת פרופיל שקדם להם לא נרשם ולא התריע
- [x] להוסיף דיווח שרת מובנה לשלב `profile_save` עם לוג מסונן שאינו שומר תמונה, שם, אימייל מלא או טלפון
- [x] להוסיף התראת SMS מיידית לבעלת האתר בכשל טכני של שמירת פרופיל או יצירת Grow, עם cooldown של 15 דקות למניעת הצפה
- [x] להוסיף בדיקות רגרסיה לסיווג כשל קריטי, SMS, cooldown ואי־שליחת SMS בכשל SDK או כרטיס רגיל
- [x] להריץ 99 קובצי בדיקה ו־439 בדיקות, TypeScript, build ו־diff; 98 קבצים ו־438 בדיקות עברו ואחד דולג כמתוכנן
- [x] לפרסם ולסנכרן ל־GitHub את מנגנון הניטור
- [x] להכין דיווח שקוף על הסיבה, חלון הזמן, מספר המושפעים ומה ישתנה מעכשיו

## אירוע תשלום חוזר לאחר תיקון — 09.09.2026 11:20

- [x] לתקן מצב שבו השרת מחזיר `success: true, alreadyExists: true` לפרופיל שכבר שילם, אך הלקוח הופך זאת למסך „לא הצלחנו לשמור” במקום לעצור חיוב כפול ולהפנות לאזור האישי
- [x] להחזיר את GrowWallet מ־CDN ישיר 1.3.6 ל־SDK המקומי 1.3.5 ול־`/api/grow-proxy`, משום שה־CDN שולח `drawWalletPageData` ישירות ל־sdk.meshulam.co.il ונחסם ב־Incapsula בעוד ה־proxy הקיים מנתב את secure.meshulam.co.il דרך fallback
- [x] להוסיף ל־createPaymentProcess בצד השרת fallback דרך Grow Worker הקיים כאשר הקריאה הישירה מחזירה 403/Incapsula, כדי שכשל WAF לא יחסום את המעבר לארנק
- [x] להסביר מדוע לא נוצר דיווח `profile_save` מהצילום: הלקוח דילג במפורש על הדיווח עבור `PROFILE_ALREADY_REGISTERED`, אף שהציג אותו בטעות ככשל שמירה
- [x] לשחזר את כל שלבי הטופס בדפדפן עם נתוני פרופיל מלאים, שאלון DNA ותמונה, ולא באמצעות קריאת API מקוצרת
- [x] לשחזר את אותו מסלול בתצוגת מובייל אמיתית ברוחב 390px; Chromium עבר. WebKit/Safari אינו זמין בסביבה ולכן תועדה המגבלה ולא נטענה תמיכה שלא נבדקה
- [x] לבדוק אם שמירת הפרופיל נכשלת עקב payload, שדה מסוים, race בין DNA לשמירה, timeout או כפילות: צילום 11:20 היה `alreadyExists` של פרופיל שכבר שילם שהלקוח הציג בטעות ככשל; בנוסף אותרה חסימת WAF נפרדת ב־createProcess וב־draw של Grow עבור נרשמים חדשים
- [x] לתקן את שורש הכשל בלי לעקוף שמירת פרופיל ובלי לאפשר מעבר ל־Grow כאשר אין רשומה תקינה
- [x] להוסיף בדיקות רגרסיה שמדמות את נתוני הטופס המלאים והמסלול שנכשל בפועל
- [x] להמתין ל־`renderPaymentOptions` האמיתי לפני ואחרי createProcess ולהגן על פתיחה ראשונה של Grow
- [x] להוסיף בדיקת רגרסיה שמונעת חזרה למצב שבו runtime חלקי נחשב מוכן לתשלום
- [x] לאמת שהארנק עצמו נעשה גלוי לאחר render; אם Grow יוצר מעטפת נסתרת בלבד, לבצע retry בטוח עם אותו authCode עד לפתיחה
- [x] להריץ בדיקות מלאות, TypeScript, build, diff וסריקת פרטיות לפני פרסום: 100 קובצי בדיקה עברו, אחד דולג כמתוכנן; 449 בדיקות עברו ואחת דולגה
- [x] להסיר כתובת אימייל מלאה מלוגי `PaymentStep` ולהשאיר רק דומיין/אינדיקציה מסוננת לפני הפרסום הסופי
- [x] לפרסם ולסנכרן ל־GitHub את תיקון החירום בגרסה `1f007884`
- [x] לאמת בדסקטופ, במסלול DNA-origin ובמובייל 390px מעבר מפרופיל מלא לארנק Grow גלוי על 299 ₪ בלי כרטיס ובלי חיוב
- [x] לאמת שכשל QA מדווח להתראה ושלא נשמרים אימייל, טלפון או טוקן מלאים בלוג; בדיקות נוספות לא יפעילו התראת SMS אמיתית
- [x] לדווח למשתמשת על שורש הכשל האמיתי בלי להסתמך על ההנחות מהדיווח הקודם

## התאמת שדה קוד חינמי ושחזור תשלומים — 09.09.2026

- [x] להעביר את שדה קוד הכניסה החינמית מראש עמוד ההרשמה לאזור התשלום בלבד, סמוך לתשלום המלא
- [x] לשמור תמיכה בקישור `free_token` ישיר ובטוקן שמודבק בשדה התשלום, בלי להציג את השדה בראש הפרופיל; בדיקת רגרסיה ממוקדת ו־TypeScript עברו
- [x] לאמת אם שלוש התראות התשלום מהשעה האחרונה היו בדיקות QA או ניסיונות אמיתיים: שמונה התראות האימייל היום סווגו כולן כ־QA, ושלושת ה־SMS היו ההתראות הקריטיות המסוננות מאותן בדיקות; לא נמצא ביניהן לקוח אמיתי
- [x] לזהות לידים אמיתיים שהתחילו תשלום למאגר אך אין להם עסקת Grow מאומתת: 0 בשעה האחרונה, 1 ב־24 השעות האחרונות ו־4 בשבעת הימים האחרונים לאחר החרגת QA ומי שכבר שילם; ל־0 מהם יש הסכמת שיווק פעילה ולכן לא הוגדרה שליחה
- [x] לוודא שהקופון LOVE10 קיים ופעיל: 10% ללא תפוגה וללא מגבלת שימוש; הוא אינו מוגבל למאגר בלבד ולכן לא שונה ללא אישור מפורש
- [x] להכין כמות נמענים ונוסח הודעת שחזור עם LOVE10 לאישור, ללא שליחה אוטומטית: נמצאו 4 ניסיונות אמיתיים ולא־משלמים בשבעה ימים, אך 0 נמענים עומדים כעת בתנאי הסכמה פעילה; הוכן נוסח לשימוש רק אם תאושר דרך פנייה תואמת־הסכמה
- [x] להריץ בדיקות מלאות, TypeScript, build ו־diff לאחר שינוי ה־UI: 100 קובצי בדיקה ו־449 בדיקות עברו, קובץ אחד ובדיקה אחת דולגו כמתוכנן; TypeScript, build ו־diff עברו
- [x] לפרסם ולסנכרן ל־GitHub בגרסה `64fb14dc`, ולאמת באתר החי שהשדה אינו מופיע בראש הפרופיל ומופיע רק באזור התשלום
- [x] לנקות את כל פרופילי, לידי וסשני ה־QA שנוצרו בבדיקות checkout: נמחקו 12 פרופילים לא־משלמים ו־48 רשומות מקושרות; ספירת השאריות היא אפס

## החזרת הודעות התאמה אוטומטיות ל־Vibrate SMS — 09.09.2026

- [x] למפות את כל נקודות השליחה האוטומטיות הקשורות להתאמות: אותרו שלושה אירועי WhatsApp פעילים בלבד — הצעה ראשונית רגילה/Boost, תזכורת לממתינים ותפוגת התאמה; אישורי צדדים ושחרור פרטים אינם שולחים WhatsApp כיום
- [x] לזהות בכל נקודת שליחה האם כיום יוצא WhatsApp, SMS, אימייל או שילוב: הצעה ראשונית ותזכורת יוצאות במייל+WhatsApp, תפוגה יוצאת ב־WhatsApp ללא־משיב ובמייל לצד שכבר אישר; ההחלפה תשאיר את המיילים ותמיר רק את WhatsApp ל־SMS יחיד
- [x] להעביר את כל הודעות ההתאמה האוטומטיות שיוצאות כיום ב־WhatsApp לשליחת SMS דרך Vibrate, בלי לשנות את המיילים הקיימים: הצעה ראשונית רגילה/Boost, תזכורת ותפוגה
- [x] להתאים את הנוסחים לאורך SMS תוך שמירת שם, אחוז התאמה והפניה ברורה למייל האישי; נוסח התזכורת תוקן כך שאינו טוען שהצד השני אישר אם הדבר אינו ידוע
- [x] לכבד הסרות ופרופילים לא פעילים או מדומים באמצעות חסימת `isActive=false` ו־`isSeed=true`; בדיקות בוצעו באמצעות mocks בלבד ולא נשלחה הודעה למספר QA או ללקוח
- [x] להוסיף בדיקות רגרסיה שמוודאות שאין קריאת WhatsApp במסלולי התאמה, שהצעה ראשונית נדרשת פעם אחת בלבד ושכל צד תקין מקבל Vibrate SMS יחיד; נוספו גם בדיקות לחסימת פרופילים לא פעילים או מדומים
- [x] להריץ בדיקות ממוקדות ומלאות, TypeScript, build ו־diff: 98 קובצי בדיקה ו־442 בדיקות עברו, קובץ אחד ובדיקה אחת דולגו כמתוכנן
- [x] לפרסם ולסנכרן ל־GitHub בגרסה `34997997`; שרת הייצור אומת ב־HTTP 200 דרך endpoint בריאות ציבורי, ולא נשלחה הודעת בדיקה ללקוח או למספר QA

## התראת תשלום חדשה לאחר תיקון — 09.09.2026

- [x] לזהות את אירועי `sdk_failure` מתוך חלון Grow בשעות 19:16–19:17: שני ניסיונות Apple Pay הגיעו ל־`chargePaymentToken` ונדחו על ידי Grow בקוד ספק 103, בנפרד משתי דחיות phone ב־createProcess של אדם אחר
- [x] לבדוק אם נוצרה עסקה או הרשאה חלקית עבור ניסיונות 19:16–19:17: נמצאו אפס תשלומים מאומתים, אפס webhooks הצלחה, והפרופיל נשאר לא־משלם ולא־פעיל; לא בוצעה פעולה כספית
- [x] לתקן את כשל התשלום בתוך הארנק עצמו: ה־proxy משחזר כעת רקורסיבית את שדות Apple Pay המקוננים ב־bracket notation במקום לשלוח `card_token=[object Object]`; במקביל נוספו נרמול וולידציית טלפון לפני Grow והפסקת התראת חירום על שגיאת קלט
- [x] לזהות את ההתראה האחרונה ולסווג אם מדובר בלקוחה אמיתית: הכשל התרחש בתוך SDK של Grow לאחר פתיחת הארנק, בשלב חיוב Apple Pay, בקוד ספק 103
- [x] לבדוק האם הפרופיל נשמר, האם נוצר תהליך Grow והאם העסקה הושלמה: הפרופיל והליד נשמרו, נוצר תהליך Grow, אך לא נרשמה עסקה מאומתת ולא התקבל webhook הצלחה
- [x] לשחזר פסיבית את ענף Apple Pay מתוך בקשת הייצור שנכשלה ולבדוק את אותו מבנה parsed body בבדיקת רגרסיה, ללא יצירת Apple Pay חדש, כרטיס או חיוב
- [x] להוסיף בדיקת רגרסיה מדויקת ל־Apple Pay nested payload, נרמול טלפון וסיווג שגיאת Grow 946; 98 קובצי בדיקה ו־446 בדיקות עברו, קובץ אחד ובדיקה אחת דולגו כמתוכנן, לצד TypeScript, build ו־diff
- [x] להסיר גוף webhook ובקשות proxy גולמיים מלוגי Grow ולהשאיר רק content-type, שמות שדות, סטטוס וקוד ספק, כדי שלא יישמרו טוקנים או פרטי תשלום בלוגי השרת
- [x] לפרסם ולסנכרן ל־GitHub בגרסה `4d950dbc`; לאחר ההפצה ה־bundle החדש אומת בדומיין החי ומסלול DNA-origin הגיע לחלון Grow גלוי על 299 ₪ ללא בחירת אמצעי תשלום או חיוב
- [x] לנקות את פרופיל ה־QA וכל הרשומות המקושרות שנוצרו באימות; השורש המוכח תועד ללא PII או טוקנים

## הפעלת מנוי Database Plus לאחר תשלום — 09.09.2026

- [x] לאתר את הפרופיל שצורף ולבדוק את מצב המאגר, Plus וה־Boost הנוכחי: הפרופיל פעיל ומשולם במאגר, מאושר Boost, אך רשומת Plus היא `invited` עם billing `not_configured`
- [x] לאמת עסקת Database Plus מוצלחת ב־Grow Production: אסמכתת Grow המצורפת מאשרת 99 ₪ עבור Database Plus; מספר העסקה לא נקלט ב־webhook/intent ולכן נדרש קישור ידני לפי הוראת בעלת האתר
- [x] לוודא שאין מנוי Plus פעיל או רשומת subscription כפולה: קיימת רשומת pilot יחידה שאינה פעילה ואין provider subscription מחובר
- [x] להפעיל את מנוי Database Plus, שתי ההתאמות החודשיות וה־Boost החינמי בהתאם להטבות השירות, ללא חיוב נוסף: המנוי וה־billing פעילים, יעד 2 התאמות ושירות בעדיפות הופעלו, וזכאות Boost פעילה
- [x] לאמת שהמנוי מופיע בנתוני הדאשבורד כפעיל ומשולם ושמצב ה־Boost החינמי תקין: intent יחיד פעיל ב־Production על 99 ₪, מנוי יחיד פעיל, יעד 2 התאמות, שירות בעדיפות ו־Boost פעיל
- [x] לתעד את הפעולה ללא מזהי עסקה או PII, לשמור checkpoint ולסנכרן ל־GitHub
- [x] לוודא שמספר האסמכתה שצורף קיים פעם אחת בלבד בתשלומים/‏webhooks ושטרם שויך למנוי Plus אחר: לא נמצאה שום רשומה קיימת עם האסמכתה בתשלומים, webhook, intent או אירועי Plus
- [x] לקשר את עסקת ה־99 ₪ לפרופיל של שי לפי האסמכתה והנחיית בעלת האתר, מבלי לשנות את פרטי הקשר של הפרופיל; נרשמו payment/event/idempotency פעם אחת בלבד

## ניתוח המרת Database Plus ושחזור חיובי 1 ₪ — 09.09.2026

- [x] למפות את כל קהל ההשקה שנשלחו אליו מייל ו־SMS ואת חלוקת הקבוצות, ללא חשיפת PII: 60 בקהל ההשקה ו־5 בקבוצת חיובי ה־1 ₪
- [x] לחשב פתיחות, הקלקות, הגעה לעמוד, יצירת intent ותשלומים: 60/60 מיילים ו־60/60 SMS נשלחו; 42 פתחו מייל, 1 הקליק במייל, 4 ביקורי עמוד מתועדים, 0 intents ו־0 רכישות מהקהל הרחב; בקבוצת השחזור 3/3 קיבלו מייל+SMS ו־2/3 השלימו 99 ₪
- [x] לזהות את צווארי הבקבוק: בקהל הרחב הירידה היא לפני התשלום — 70% פתיחה אך 1.7% הקלקה ורק 6.7% ביקורי עמוד; בקבוצת כוונה גבוהה ההמרה הייתה 66.7%, ולכן עיקר הבעיה הוא המסר/הצעת הערך והמעבר לעמוד ולא כשל checkout כולל
- [x] לאתר את כל מי שחויבו 1 ₪ ולהחריג את שי, משלמי 99 ₪, מסירים ומי שכבר קיבלו SMS: לאחר הסינון נשאר אדם זכאי אחד למעקב; שניים חסומים או ללא הסכמה ואדם נוסף כבר שילם
- [x] לאמת את מספר הנמענים הסופי ואת נוסח ה־SMS עם קישור Production תקין לפני שליחה: נמען אחד, תזכורת קצרה, קישור אישי והסרה
- [x] לשלוח SMS שחזור פעם אחת בלבד לזכאי דרך Vibrate ולתעד idempotency; לא בוצע חיוב ולא הופעל Plus לפני תשלום Production מאומת
- [x] לאמת קבלת SMS אצל הספק: הודעה אחת התקבלה אצל Vibrate, 0 נדחו; הדיווח נשמר ללא PII

## התראת תשלום חדשה ובדיקת היקף — 09.09.2026

- [x] לאתר את התראת התשלום האחרונה אצל ספק ההתראות: timeout של Grow renderer אחרי 12 שניות בניסיון הראשון
- [x] לזהות את הפרופיל שנפגע באופן מסונן ולהצליב: הפרופיל נשמר, הניסיון השני פתח Grow והסתיים בתשלום מאגר מאומת של 299 ₪ ובפרופיל פעיל; אין צורך בקישור חדש
- [x] לבדוק את כל ניסיונות התשלום מאז התיקון האחרון: 7 תשלומי מאגר מאומתים; התראה נוספת אחת הייתה דחיית מספר כרטיס לא תקין בתוך Grow ללא חיוב
- [x] לקבוע את סוגי הכשל: האירוע האחרון הוא readiness איטי של Grow SDK בקליק הראשון; האירוע הנוסף הוא שגיאת קלט בכרטיס ולא תקלה באתר
...
- [x] הפתרון אומת ללא חיוב נוסף, קובצי QA נוקו והמשך התשלומים החי אומת
...
## שחרור התאמה מהאזור האישי — 11.09.2026

- [x] למפות את פעולת השחרור באזור האישי ואת כל סטטוסי ההתאמה והפרופילים שהיא אמורה לעדכן: האזור האישי קורא `matchmaking.returnToPool`, בעוד ה־CRM משתמש במסלול מלא יותר וב־`getReleaseLifecycleUpdate`
- [x] לשחזר את המקרה שבו המשתמשים נשארים מוצגים כזוג לאחר שחרור ולזהות את שורש התקלה: `returnToPool` עדכן רק `returnedToPoolAt`, לא אימת את המשתמש ולא החיל `matchDetailStatus=ended`; האזור האישי והיסטוריית ה־CRM המשיכו להציג `status=matched` כזוג. נמצאו 87 רשומות היסטוריות עם lifecycle חלקי, ובדיקת הרגרסיה נכשלה בארבעת הענפים לפני התיקון
- [x] לתקן את השחרור כך ששני הצדדים משתחררים בעדכון אטומי של רשומת ההתאמה, חוזרים למאגר והיסטוריית ההתאמה נשמרת; הפעולה מאמתת אימייל, טוקן ושייכות להתאמה ומחילה `matchDetailStatus=ended`
- [x] להוסיף הודעת אישור ברורה לאחר שחרור מוצלח והודעת שגיאה אם הפעולה לא הושלמה; באזור האישי ובהיסטוריית ה־CRM מוצג כעת „שוחררו” במקום „זוג”
- [x] לאתר ולתקן באופן ממוקד רשומות קיימות ששוחררו אך עדיין מופיעות כזוג, בלי לגעת בזוגות פעילים אמיתיים: 87 רשומות עם `returnedToPoolAt` ו־lifecycle חלקי עודכנו ל־`matchDetailStatus=ended`; לאחר התיקון נשארו 0 רשומות חלקיות
- [x] להוסיף בדיקות רגרסיה למסלול האזור האישי, קישורי המייל ומצבי CRM/אלגוריתם לאחר שחרור; 8 בדיקות ממוקדות עברו
- [x] להריץ בדיקות מלאות, TypeScript, build ו־diff: 99 קובצי בדיקה ו־451 בדיקות עברו, קובץ אחד ובדיקה אחת דולגו כמתוכנן; TypeScript, build ו־diff עברו
- [x] לפרסם את התיקון ולסנכרן אותו ל־GitHub: הפרסום החוזר ב־`62e16252` הצליח לאחר כשל Cloud Build זמני, וה־bundle החי מכיל את הודעות השחרור החדשות
- [x] לאמת באתר החי שהשחרור עובד מקצה לקצה בלי למחוק היסטוריה או לשלוח הודעה ללקוחות: זוג QA שוחרר דרך הכפתור, הוצגה הודעת הצלחה, מספר ההתאמות הפעילות ירד ל־0, ההתאמה עברה להיסטוריה כ„שוחררה”, שני הפרופילים נשארו פעילים, וה־QA נמחק במלואו

## ביטול מייל „הפרופיל שלך פעיל” — 11.09.2026

- [x] לאתר את המייל לפי הכותרת, הטקסט „עד עכשיו נשלחו עבורך … הצעות התאמה”, ה־journey והתבנית בכל קוד השרת והלקוח: המקור הוא שלב 3 במסע `database_90_day_v1` בקובץ `server/database90DayJourney.ts`
- [x] לאתר טריגרים מתוזמנים, Heartbeat, cron, webhook או מסע אוטומציה שיכולים לשלוח את המייל: endpoint ה־Heartbeat היומי `/api/scheduled/database-90-day-journey` מפעיל את המסע ישירות ושולח דרך Brevo
- [x] להיכנס למערכת המיילים האוטומטיים ולסרוק מסעות פעילים, תבניות שמורות וגרסאות דומות של „הפרופיל שלך פעיל” או מייל עם ספירת הצעות: נסרקו דרך Brevo API כל התבניות והקמפיינים בחשבון; נמצאו 0 תבניות פעילות, 0 קמפיינים דומים ו־0 גרסאות ספק נוספות. מקור השליחה היחיד הוא מסע האתר `database_90_day_v1`
- [x] לאתר הודעות pending/processing קיימות מהסוג הזה ולבטל אותן בלי לגעת במיילי התאמה ספציפיים: נסרקו 10,000 רשומות המייל האחרונות ונמצאו 0 הודעות pending או processing של `database_90_day_v1` או של נוסח דומה; לא היה תור קיים לביטול
- [x] להסיר לחלוטין את כל נקודות השליחה של המייל ואת חישוב מספר ההצעות המוצג בו: שלבי יום 14, 30, 60 ו־90 הוסרו מהמסע, נשארו רק יום 3 להשלמת פרופיל ויום 7 להסבר השירות, וכל שלב מאוחר יותר נחסם ב־`generic_status_email_disabled`
- [x] להוסיף בדיקת רגרסיה שמונעת מהכותרת, הטקסט ושלבי יום 14–90 לחזור לקוד או לתור; 7 בדיקות ממוקדות עברו
- [x] להריץ בדיקות מלאות, TypeScript, build ו־diff: 99 קובצי בדיקה ו־449 בדיקות עברו, קובץ אחד ובדיקה אחת דולגו כמתוכנן; TypeScript, build ו־diff עברו
- [x] לפרסם, לסנכרן ל־GitHub ולאמת ללא שליחת מייל ללקוחות: גרסה `57f8b768` חיה, האתר מחזיר 200, לוגי הייצור נקיים משגיאות טעינה, ולא הופעל endpoint המסע במסגרת האימות

## תרגום הרצאה על נדידת מחשבות ומדיטציה — 11.09.2026

- [x] לקרוא את כל 26 פסקאות ההרצאה ולוודא שאין מקטע חסר
- [x] לתרגם לעברית טבעית ונאמנה תוך שמירת המבנה, הדוגמאות והמונחים המקצועיים
- [x] לבצע בדיקת שלמות ועריכה לשונית: התוצר כולל 26 פסקאות תואמות למקור, מונחים עקביים וקובץ Markdown מסודר

## תרגום שאלות הקורס ותשובות מומלצות — 11.09.2026

- [x] לזהות ולתרגם את שאלות 1–4 ו־6–9 ואת כל אפשרויות התשובה בשני הקבצים
- [x] להוסיף לכל שאלה תשובה מומלצת והסבר קצר המבוסס על תוכן ההרצאות
- [x] לציין במפורש ששאלה 5 חסרה וששאלה 9 היא שאלה אישית ללא תשובה נכונה

## תיקון קישורי חבילת החג ללקוח משלם — 11.09.2026
- [x] לאמת עסקת Grow מאומתת בסך 399 ₪ עבור חבילת החג ולוודא שהזכאות משויכת ללקוח הנכון: נמצאה עסקת Grow יחידה ואמיתית למוצר `bundle_new_year`, ללא חיוב נוסף
- [x] לאתר את מייל „חבילת החג שלך מוכנה” שנשלח ואת שני הקישורים שהוטמעו בו: Brevo אישר request, delivery, פתיחות ולחיצות לשני נתיבי המוצר הנכונים
- [x] לבדוק מדוע קישור הקורס וקישור המדריך אינם עובדים ולוודא שאין בעיית token, redirect, הרשאה או כתובת ישנה: שני הטוקנים באורך תקין, לא פגו, נוצרו יחד עם הרכישה והובילו לנתיבים החיים הנכונים; לא שוחזרה תקלה מערכתית ולא נקבעה סיבה לא מוכחת
- [x] ליצור או לשחזר קישור גישה מאובטח ותקין לקורס „המסע לזוגיות” ולקישור המדריך „לבחור נכון”, ללא חיוב נוסף: נעשה שימוש בטוקנים הקיימים והתקינים כדי לא ליצור כפילויות או לפגוע בהתקדמות קיימת
- [x] לבדוק את שני הקישורים בפועל עד למסך התוכן המורשה, בלי לשנות נתוני רכישה או לחשוף טוקנים: Chromium מבודד טען בדומיין החי את כותרת הקורס ואת מודול 1, וכן את כותרת המדריך ואת פרק 1, ללא מסך „קישור לא תקף”
- [x] לשלוח ללקוח מייל מתוקן אחד עם שני הקישורים ולאמת שהספק קיבל את ההודעה: נשלח מייל תיקון עם שני כפתורים ושתי כתובות ישירות גלויות להעתקה; Brevo אישר request ו־delivery להודעה יחידה
- [x] לתקן את מקור הקישורים עבור רוכשי חבילת חג עתידיים אם התקלה מערכתית, ולהוסיף בדיקת רגרסיה: לא בוצע שינוי קוד משום שהנתיבים, הטוקנים, ההרשאות ותבנית המייל נבדקו ונמצאו תקינים ולא שוחזרה תקלה מערכתית
- [x] לתעד ללא PII, לשמור checkpoint ולסנכרן ל־GitHub

## ניתוח מצב הקמפיינים והמכירות בשלושת השבועות האחרונים — 13.09.2026
- [x] להגדיר חלון ניתוח מדויק ושלוש תקופות השוואה הוגנות לפי ימים ושעות: 23.8–12.9, שלושה שבועות מלאים בני שבעה ימים לפי שעון ישראל; היסטוריית התשלומים המאומתים המוקדמת יותר סומנה כחלקית ולא שימשה בסיס כספי מטעה
- [x] לאסוף במקביל הכנסות מאומתות, יחידות מכירה, לידים, ניסיונות תשלום, נטישות ותקלות לפי מוצר ויום, ללא PII ובסינון 11 עסקאות ניסיון של 1 ₪
- [x] לאסוף במקביל נתוני Meta Ads לפי קמפיין, קהל, קריאייטיב, הוצאה, CPM, CTR, CPC, לידים, רכישות, CPA ו־ROAS משני חשבונות הפרסום
- [x] לנתח תנועה והתנהגות באתר לפי עמוד, מקור, UTM, מכשיר ושלב במשפך, כולל הבחנה בין ירידה לפני התשלום לבין שיעור השלמה יציב לאחר פנייה לתשלום
- [x] למפות את כל שינויי הקוד, העמודים, המחירים, ההצעות, הקופונים, המיילים והקמפיינים שבוצעו בתקופה, כולל 208 commits ו־261 קבצי production שנגעו בהם
- [x] להשוות את שלושת השבועות לתקופות בסיס קודמות תוך נטרול חלקיות ימים, סופי שבוע וחגים; תקופות היסטוריות עם ledger חלקי הוחרגו מהשוואת הכנסה
- [x] לבדוק עונתיות ומצב שוק רלוונטי בישראל בתקופה מול מקורות חיצוניים אמינים, כולל הלמ״ס, תאריכי ראש השנה, הוצאות חזרה ללימודים ומתחרים
- [x] להפריד בין עובדות, מתאמים, השערות וגורמים סיבתיים סבירים באמצעות מטריצת רמת ביטחון ומשקל
- [x] לגבש תוכנית פעולה מיידית ל־72 שעות, תוכנית ל־14 יום ומדדי עצירה או הגדלה
- [x] להכין דוח עברי ברור עם טבלאות, חמישה תרשימים, מסקנות וסדרי עדיפויות, כולל תצוגה חזותית מסויגת של Similarweb
- [x] לתעד ללא PII, לשמור checkpoint, לפרסם ולסנכרן ל־GitHub

## החזרת אות Purchase מאומת ואיפוס הקמפיינים — 13.09.2026
- [x] למפות את מסלול Grow המלא מפתיחת תשלום עד webhook, עמוד תודה, CAPI ו־Pixel בכל המוצרים: נמצא שהמאגר נשען על CAPI מאומת אך עם מזהי התאמה חסרים, בעוד שישה עמודי תודה אחרים עדיין ירו Purchase לא מאומת בעת טעינת העמוד
- [x] להגדיר חוזה Purchase מאומת שאינו נורה מעצם פתיחת עמוד תודה ומשתמש ב־event_id אחיד למניעת כפילות: token אקראי נוצר ב־createProcess, נשמר עם מזהי הדפדפן, מסומן paid רק ב־webhook, ונצרך פעם אחת בדפדפן לצורך Pixel עם אותו event_id של CAPI
- [x] לשמור בפתיחת התשלום את fbp, fbc, user agent והקשר הייחוס הנדרש באופן מאובטח ומוגבל ל־24 שעות
- [x] להעביר ל־Meta CAPI את מזהי ההתאמה החסרים רק לאחר תשלום Grow מאומת
- [x] להחזיר אירוע Purchase בדפדפן רק לאחר אישור שרת לעסקה, עם אותו event_id של CAPI ו־idempotency אטומי
- [x] להוסיף בדיקות רגרסיה שמונעות Purchase כוזב, כפילות, חשיפת PII או שימוש חוזר באישור עסקה
- [x] להריץ בדיקות ממוקדות, pnpm test, pnpm check, pnpm build ו־git diff --check: לאחר הוספת הניטור 455 בדיקות עברו, בדיקה אחת דולגה כמתוכנן
- [x] לאמת את מסלול המדידה מקומית ובייצור ללא ביצוע חיוב אמיתי: עמודי התודה נטענו, endpoint האישור החזיר `invalid` לטוקן לא מוכר, ולוגי הייצור היו נקיים משגיאות tracking
- [x] לבצע לאחר אימות המדידה את איפוס התקציבים והקמפיינים שאושר, בלי למחוק היסטוריה: הוחלו 12 שינויים הפיכים ב־Meta, ללא מחיקת קמפיינים או מודעות
- [x] לאמת שהקמפיינים הפעילים והתקציבים בפועל תואמים לתוכנית 72 השעות: DNA קר 200 ₪, DNA חם 50 ₪, מאגר קר 60 ₪ ומאגר חם 60 ₪, סך הכול 370 ₪ ליום; באנדל, Boost, Traffic וקידומי פוסטים נעצרו
- [x] לתעד, לשמור checkpoint, לפרסם, לסנכרן ל־GitHub ולעקוב אחר אות Purchase מול Grow: דוח חצות כולל כעת שורת Grow מול Meta עם יחס קליטה ומצב תקין, במעקב או דורש בדיקה

## החלפת צדדים בבדיקת התאמה ספציפית — 13.09.2026
- [x] לאתר את ההתאמה האחרונה בין שני הפרופילים בלי לחשוף PII ולבדוק את צד א׳, צד ב׳, סטטוס וטוקני התגובה: אותרה רשומה יחידה, שני הטוקנים נפתחו ונצרכו ושני הצדדים אישרו
- [x] לבדוק אילו קישורי התאמה והודעות נשלחו לכל צד ומתי, בלי לשלוח הודעה נוספת במהלך האבחון: הוכח שטוקן A מציג תמיד את צד B וטוקן B מציג תמיד את צד A, בעוד מסלול השליחה בנה את הנמען לפי סדר הבחירה במסך
- [x] לשחזר את מסלול „בדיקת התאמה ספציפית” בקוד ובממשק ולזהות היכן צד המקבל וצד הפרופיל מתחלפים: createAndSendMatch מצא רשומה קיימת גם בכיוון ההפוך אך המשיך לבנות את המיילים לפי idA/idB במקום לפי singleAId/singleBId השמורים
- [x] לבדוק מקרים קודמים ולהעריך את היקף הבאג בלי לשנות התאמות תקינות: בחלון 14 יום נמצאו 20 רשומות עם פער יצירה־שליחה, אך רק המקרה הנוכחי נשא את חתימת מסלול הבדיקה הידנית ללא נתוני אלגוריתם או אישור מוקדם
- [x] לתקן את הקוד כך שכל צד יקבל תמיד את פרופיל הצד השני בכל ערוצי השליחה: שימוש חוזר ברשומה קיימת מיישר כעת את סדר הבחירה ל־singleAId/singleBId השמורים לפני בניית המיילים, הטוקנים וה־SMS
- [x] לתקן בבטחה את רשומת ההתאמה הנוכחית ולבטל קישורים שגויים רק אם הדבר נדרש: הרשומה עצמה הייתה תקינה, שני הצדדים כבר אישרו ומיילי פרטי הקשר הנכונים נמסרו לשניהם, ולכן לא בוצע שינוי נתונים ולא נוצרו טוקנים חדשים
- [x] להוסיף בדיקות רגרסיה לשני כיווני ההתאמה, לטוקנים ולמסלול בדיקה ספציפית: נבדקו סדר רגיל, סדר בחירה הפוך ודחיית זוג שאינו שייך לרשומה
- [x] להריץ בדיקות ממוקדות, pnpm test, pnpm check, pnpm build ו־git diff --check: 458 בדיקות עברו, בדיקה אחת דולגה כמתוכנן
- [x] לפרסם, לסנכרן ל־GitHub ולאמת חי ללא שליחת הודעת בדיקה ללקוחות: גרסה cb5ed866 פורסמה, GitHub main סונכרן והדומיין החי החזיר HTTP 200; לוגי Cloud Run לא היו זמינים זמנית ולכן האימות נשען על סטטוס הפריסה והדומיין החי
- [x] לשלוח מחדש רק לצד שנפגע, אם יאומת שנדרש, ולוודא שקיבל את פרופיל הצד השני: לא נדרשה שליחה מחדש משום ששני הצדדים כבר לחצו כן ומיילי חשיפת הפרטים הנכונים נמסרו לשניהם; נמנעה הודעה כפולה

## אימות מייל וכניסה לאזור האישי — 13.09.2026
- [x] לאתר לפי המייל והטלפון שבצילום את כל רשומות הפרופיל, CRM, שאלונים ותשלומים בלי להדפיס PII: אותר פרופיל מאגר יחיד ורשומות CRM, הזמנה והמלצה הקשורות אליו
- [x] לוודא שאין כפילות או כתובת ישנה שמשמשת באחד ממסלולי הכניסה או הדיוור: נמצאה כתובת ישנה בשלוש רשומות תפעוליות ובלוגי מסירה היסטוריים בלבד
- [x] לוודא שהמייל שבצילום הוא המייל הראשי ברשומת המאגר הפעילה: הכתובת תואמת בדיוק, והפרופיל פעיל, משולם והשאלון הושלם
- [x] לוודא שטוקן האזור האישי משויך לאותה רשומה ולאותה כתובת: קיים טוקן יחיד באורך 64 תווים המשויך לפרופיל היחיד
- [x] לתקן שיוכים או כתובות רק אם נמצא פער, בלי למחוק היסטוריה: עודכנו טרנזקציונית רשומת CRM אחת, רשומת הזמנה משומשת אחת ורשומת המלצה ארכיונית אחת; לוגי המסירה ההיסטוריים נשמרו ללא שינוי
- [x] לאמת בדומיין החי שהקישור האישי נפתח לפרופיל הנכון, בלי לחשוף את הטוקן: endpoint החי החזיר את אותו profile id ואת המייל הנכון ב־HTTP תקין
- [x] לנקות קובצי ביקורת זמניים ולתעד את התוצאה ללא PII

## בריף UGC לסיפורי הצלחה ומיצוב אוטוריטה — 14.09.2026
- [x] להגדיר מטרות, קהלים, טון וכללי אמינות והסכמה לפרסום
- [x] לבנות מטריצת ליהוק מאוזנת לפי מגדר, גיל, אזור, מצב חיים וסוג חוויה
- [x] לחלק את הסרטונים לצירי תהליך אישי, מאגר, כלים ושיטת ההתאמה
- [x] לנסח את סיפור המותג והמסרים שממצבים את הילית כאוטוריטה בתחום האהבה והשידוכים
- [x] לכתוב מבני סרטון, הוקים, שאלות מנחות, משפטי הוכחה וקריאות לפעולה
- [x] להוסיף הנחיות צילום, עריכה, כתוביות, אורכים וגרסאות לקהל חם וקר
- [x] להכין בריף סופי בעברית למסירה, ללא עדויות מומצאות או טענות שאינן ניתנות לאימות

## שליחה חוזרת של התאמה לנמען יחיד — 15.09.2026
- [x] לאתר את רשומת ההתאמה עם שרית ואת המייל והטוקן שנוצרו עבור הנמען המבוקש: נמצאה רשומה יחידה במצב `proposed`, עם טוקן תקף שטרם נוצל
- [x] לבדוק אצל ספק המייל אם ההודעה התקבלה, נמסרה, נפתחה, נדחתה או נחסמה: Brevo קיבל את ההודעה ותיעד שתי פתיחות, ללא bounce, block או blacklist; ייתכן שמדובר בסריקת פרטיות אוטומטית ולא בפתיחה אנושית
- [x] לאמת שהטוקן של הנמען תקף ושקישור ההתאמה מציג את שרית ולא את הפרופיל שלו: בדיקת tRPC חיה החזירה HTTP 200, את הנמען הנכון ואת שרית כפרופיל ההתאמה, ללא תפוגה או תגובה קודמת
- [x] לשלוח מחדש מייל התאמה אחד רק לנמען המבוקש, בלי לשלוח לשרית ובלי ליצור התאמה חדשה: נעשה שימוש בטוקן הקיים ובמפתח idempotency, ונרשם retry כדי למנוע שליחה אוטומטית כפולה
- [x] לאמת קבלת ההודעה אצל הספק, לנקות קבצים זמניים ולתעד ללא PII: Brevo תיעד `requests` חדש ולאחריו `delivered` עבור הודעת התיקון היחידה

## בדיקת מייל וכניסה לאזור האישי — 15.09.2026
- [x] לאתר את הפרופיל היחיד של מיכל ואת כל כתובות המייל המקושרות אליו: נמצא פרופיל יחיד, פעיל ומשולם, ורשומת ה־CRM מקושרת לאותו profile id
- [x] לבדוק אם הכתובת שנמסרה תקינה או שהיא טעות כתיבה לעומת הכתובת השמורה: הכתובת שנמסרה אינה כתובת מייל תקינה, אינה קיימת במערכת ושונה גם בספרות וגם בסימן `@` מהכתובת השמורה
- [x] לוודא שקיים טוקן אישי תקף ושמסלול בקשת קישור הכניסה מזהה את המייל הנכון: קיים טוקן יחיד באורך 64 תווים לצד מייל תקין וזהה בפרופיל וב־CRM
- [x] לתקן כתובת, שיוך או טוקן רק אם נמצא פער, בלי למחוק היסטוריה: לא נדרש שינוי נתונים משום שהכתובת התקינה כבר אחידה והטוקן משויך אליה
- [x] לאמת בדומיין החי שהכניסה במייל הנכון מחזירה את הפרופיל של מיכל: `singles.getDashboard` החזיר HTTP 200 ואת הפרופיל הנכון עם התאמת מייל מלאה
- [x] לנקות קובצי ביקורת זמניים ולתעד ללא PII

## תיקון מלא של המייל, הכניסה וקבלת ההתאמות של מיכל — 15.09.2026
- [x] להגדיר את המייל שמופיע בצילום כמייל היחיד והקובע בכל הרשומות הפעילות: הפרופיל היחיד ורשומת ה־CRM מחזיקים באותה כתובת תקינה
- [x] להתעלם מכתובות היסטוריות לצורך כניסה, התאמות ושליחות עתידיות, בלי למחוק לוגי מסירה ארכיוניים: כל הבדיקות והתיקונים בוצעו לפי כתובת המייל הקובעת בלבד
- [x] לבדוק מדוע בקשת קישור כניסה או פתיחת הקישור נכשלת אצל מיכל למרות שקיים טוקן: ארבעה קישורי כניסה קודמים נמסרו ונלחצו עם המייל והטוקן הנכונים; ה־API והעמוד החי עובדים ולכן לא שוחזרה תקלה מערכתית, אך נשלח קישור חדש כדי לעקוף קישור ישן או בעיית דפדפן
- [x] לבדוק את ההתאמה הפעילה עם אלי, את הטוקן של מיכל ואת אירועי מסירת המייל וה־SMS שלה: נמצאה התאמה פעילה יחידה של 84%, הטוקן תקף, ה־SMS סומן כנשלח ושני מיילי ההתאמה הקודמים נמסרו ונלחצו
- [x] לבדוק אם התאמה פעילה חוסמת הצעות נוספות כמתוכנן או אם קיים באג שמונע ממנה לקבל את ההתאמה עצמה: התאמה פעילה חוסמת הצעות נוספות כמתוכנן, אך נמצא שהפרופיל הוצא ממנוע ההתאמות העתידי בגלל `consentMatchmaking=false`
- [x] לתקן את המייל, הטוקן והשיוכים הפעילים לפי הממצאים בלי למחוק היסטוריה: הרשאת ההתאמות הופעלה בפרופיל היחיד, המייל והטוקנים הקיימים נשמרו ללא שינוי
- [x] לאמת בדומיין החי כניסה לאזור האישי והצגת ההתאמה הפעילה באמצעות המייל הקובע: tRPC ו־Chromium החזירו את הפרופיל ואת אלי בלשונית ההתאמות, ללא מסך קישור לא תקין או שגיאת runtime
- [x] לנקות קובצי ביקורת זמניים, לתעד ללא PII ולסנכרן ל־GitHub אם נדרש checkpoint
- [x] לשלוח למייל הקובע בלבד קישור כניסה חדש ומייל התאמה חדש עם אלי, בלי ליצור התאמה חדשה: Brevo אישר `delivered` לשתי ההודעות החדשות

## ניתוח קמפיינים ומכירות מאז איפוס התקציבים — 15.09.2026
- [x] לקבוע את זמן איפוס Meta ואת חלון ההשוואה המקביל לפני השינוי לפי שעון ישראל: 13.09.2026 בשעה 14:02, שני חלונות זהים של 52.03 שעות
- [x] לאסוף הכנסות ורכישות Grow מאומתות לפי מוצר וזמן, ללא עסקאות ניסיון או נתוני לקוחות: 19 עסקאות ו־4,256.50 ₪ אחרי לעומת 16 עסקאות ו־3,328.50 ₪ לפני
- [x] לאסוף הוצאה, חשיפות, קליקים, לידים, רכישות, CPA ו־ROAS מכל הקמפיינים בשני חשבונות Meta: 825.10 ₪ אחרי לעומת 2,856.12 ₪ לפני
- [x] לייחס כל רכישה לקמפיין, קבוצת מודעות, מודעה, UTM, מייל או מקור אורגני ברמת הביטחון האפשרית, תוך הפרדה בין last click, השפעה מסייעת ומקורות לא ממומנים
- [x] לבדוק התאמה בין רכישות Grow לאירועי Purchase שנקלטו ב־Meta מאז תיקון המדידה: 18 מתוך 19 עסקאות קיבלו event_id ו־Meta דיווחה על 8 רכישות בשווי 2,313.90 ₪
- [x] להשוות לפני ואחרי השינוי תוך נטרול חלון חלקי ועיכוב הייחוס של Meta באמצעות פירוט שעתי לפי אזור הזמן של כל חשבון
- [x] לזהות אילו קמפיינים הניבו מכירות ואילו רק הוציאו תקציב: המאגר החם הוביל; קמפייני Boost וקידומי הפוסטים שנעצרו לא הוציאו תקציב נוסף
- [x] לגבש המלצה מה להשאיר, לעצור או להגדיל, בלי לבצע שינוי תקציב לפני אישור: להשאיר את מבנה 370 ₪ עד השלמת 72 שעות ואז לשקול העברת 20 ₪ מהמאגר הקר ל־DNA חם ולמאגר חם
- [x] להכין סיכום עברי קצר עם טבלת הוצאה, הכנסה, ROAS והחלטה מוצעת

## קישור המשך להרשמה החלקית של סלעית — 16.09.2026
- [x] לאתר לפי המייל את כל רשומות ההרשמה, הלידים, השאלונים והתשלום בלי להדפיס PII: נמצאו פרופיל יחיד, ליד CRM יחיד, טיוטת תשלום אחת וללא עסקה מאומתת
- [x] למפות אילו שדות ושלבים כבר נשמרו ובאיזה שלב נעצרה: נשמרו 22 שדות כולל פרטים אישיים, העדפות, טקסטים ותמונה; היא נעצרה לאחר פתיחת תהליך התשלום ולפני חיוב
- [x] לוודא שאין כפילות שעלולה לגרום לפתיחת הרשמה חדשה או לאיבוד נתונים: קיימת רשומת singles יחידה המקושרת לליד היחיד, ללא תשובות שאלון מדעי וללא פרופיל כפול
- [x] לאתר או להפיק טוקן המשך המשויך לאותה רשומה קיימת: נעשה שימוש בטוקן האקראי הקיים של אותה טיוטה, ונוסף נתיב resume שמוגבל לפרופיל לא משולם ואינו יוצר רשומה חדשה
- [x] לאמת בדומיין החי שהקישור מחזיר אותה לשלב הנכון עם הנתונים שכבר מילאה: ה־API החי החזיר את אותה טיוטה ו־Chromium מבודד הציג ישירות את מסך התשלום על 299 ₪ ללא טופס אישי וללא חיוב
- [x] לנקות קובצי ביקורת זמניים ולמסור קישור אישי ישיר להעתקה

## סטורי המלצות על המאגר והליווי — 16.09.2026
- [x] לקבל מהמשתמשת את חומרי ההמלצות והדירוג: סופקו מסרי ההמלצות ואושר דירוג של 5 כוכבים
- [x] לבחור שלוש המלצות שסופקו, דירוג 5 כוכבים ומסר עליון קריא לסטורי 9:16, עם ייחוס אנונימי כשאין שם אמיתי מאומת
- [x] לעצב סטורי בהשראת מבנה הרפרנס, בנראות המקורית של הילית ולא כהעתקה
- [x] לשלב כחול עמוק, קרם, ורוד עדין, טיפוגרפיה אלגנטית ומיתוג ברור של הילית כספי
- [x] לבדוק עברית, שמות, דירוג 5 כוכבים, קריאות והיעדר פרטים אישיים שלא אושרו לפרסום
- [x] למסור קובץ סטורי סופי ביחס 9:16 וברזולוציה גבוהה מ־1080×1920, מוכן לפרסום

## פער הוצאות Meta בין Ads Manager לדשבורד — 16.09.2026
- [x] להגדיר את טווח החודש המדויק לפי שעון ישראל ואת כל חשבונות Meta שהדשבורד כולל: 01.09–16.09, חשבון קמפיינים ראשי וחשבון קידומי פוסטים / Boosts
- [x] לבדוק בקוד כיצד הדשבורד מחשב total spend והאם הוא סוכם ברמת חשבון, קמפיין, מודעה או כמה רמות יחד: הדשבורד מחבר את שני החשבונות ברמת קמפיין; לא נמצאה ספירה כפולה בין רמות
- [x] לשלוף מ־Meta את ההוצאה מתחילת החודש ברמת חשבון לכל חשבון פרסום בנפרד: 11,380.27 ₪ בחשבון הראשי ו־2,052.95 ₪ בחשבון קידומי הפוסטים
- [x] להשוות את הסכום לדשבורד ולזהות אם הפער נובע מחשבון Boost, קידומי פוסטים, טווח זמן, אזור זמן, מטבע או ספירה כפולה: החשבון השני מסביר 2,052.95 ₪, ובאג UTC הוסיף 719.18 ₪ מ־31 באוגוסט והעלה את הדשבורד ל־14,152.40 ₪
