...
- [ ] למנוע חיוב כשהמועמד אינו זמין ולתכנן זיכוי או קרדיט אוטומטי אם הכשל מתגלה לאחר החיוב
...
- [ ] לקבל מ־Grow pageCode ומיפוי webhook ייעודיים ל־Match Boost לפני פתיחת התשלום
...
- [ ] לבנות מסע בקשת משוב ועדות לאחר תוצאה חיובית אמיתית בהתאמה
- [ ] לשמור בחירת הרשאה נפרדת לפרסום אנונימי, שם פרטי, שם מלא או שם ותמונה
- [ ] לדרוש אימות צוות לפני שכל עדות יכולה להופיע באתר או להיכנס לספר הקריאייטיבים
- [ ] ליצור מכל עדות מאושרת 8–12 וריאציות טקסט ל־Story, Feed, Reel, קרוסלה, ניוזלטר ומודעה
- [ ] להכין תבניות צילום מסך מעוצבות שאינן מתחזות להודעה פרטית ולא כוללות לקוח או שם מפוברקים
- [ ] לאפשר תרחישי המחשה רק עם תווית ברורה ״המחשה״ ולא תחת כותרת של עדות או סיפור לקוח
...
- [ ] להציג להילית את כל הנכסים לאישור לפני העלאה לקמפיין
...
- [ ] לאמת את מספר רכישות המאגר ביולי ובאוגוסט ואת הוצאות Meta הרלוונטיות למאגר, בנפרד ממוצרים אחרים
- [ ] לחשב יעד מינימום של 350 יחידות ויעד עבודה של 400 יחידות מאגר במחיר 299 ₪
- [ ] להגדיר תקציב מאגר של כ־10,000 ₪, CPA נדרש, ROAS נדרש וקצב רכישות יומי
- [ ] להפריד בין יעד מאגר ישיר, באנדל חג, Boost ו־Plus כדי למנוע ספירה כפולה וקניבליזציה
- [ ] לעדכן את טבלת יעדי המוצרים, חלוקת התקציב, חבילת מנהל הקמפיינים ותבניות הדיווח
- [ ] להגדיר למנהל הקמפיינים ספי שמירה, שיפור, עצירה וסקייל מול יעד 350 עד 400 יחידות
...
- [ ] להשאיר את החיוב חסום עד קבלת pageCode ייעודי, הגדרת חיוב חוזר, webhook וביטול מ־Grow
- [ ] לאחר קבלת פרטי Grow, לבדוק תשלום ראשון, חידוש, כשל חיוב, ביטול ועדכון סטטוס Plus
...
- [ ] לא לשלוח לקבוצות לפני אישור הילית לנוסח הסופי ולמועד הפרסום
