...
export default function GrowWallet({
  ...
  const growEnvironment = product === "plus" ? "DEV" : "PRODUCTION";
  ...
  const handlePay = useCallback(async () => {
    ...
    if (product === "plus") {
      ...
      try {
        const response = await fetch("https://hilitcaspi.com/api/plus/recurring-mandate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fullName, email: userEmail, phone: userPhone, consent: true }),
        });
        const result = await response.json() as { url?: string };
        if (!response.ok || !result.url) throw new Error("לא התקבלה כתובת לטופס המאובטח");
        window.location.assign(result.url);
        return;
      } catch (error) {
        setWalletLoading(false);
        toast.error(error instanceof Error ? error.message : "לא הצלחנו לפתוח את דף התשלום המאובטח.");
        return;
      }
    }
  ...
  }, [name, email, phone, product, termsPath, termsAccepted, ageConfirmed, personalToken, boostMatchId, couponApplied, growEnvironment]);
      ...
      {termsPath && (
        <div className="flex items-start gap-2 mb-3">
          ...
          <label htmlFor={`gw-terms-${instanceId}`} className="text-sm cursor-pointer leading-snug" style={{ color: '#191265' }}>
            ...
            {product === "match_boost" && " כולל קבלת הצעות Boost אלגוריתמיות, שליחת שם, תמונה ופרטי פרופיל לשני הצדדים בעת שליחת ההצעה, ואישור נפרד של כל צד"}
          </label>
        </div>
      )}
