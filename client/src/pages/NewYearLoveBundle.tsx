...
function Countdown() {
  ...
  return (
    <div className="flex items-center justify-center gap-2" aria-label="זמן נותר עד לסיום ההטבה">
      {units.map((unit) => (
        <div key={unit.label} className="min-w-13 rounded-xl border border-[#efc8b6]/35 bg-[#8a3828]/28 px-2 py-2.5 text-center shadow-[0_8px_24px_rgba(64,21,13,0.18)] backdrop-blur-sm">
    ...
    <div className={`relative overflow-hidden rounded-[1.75rem] border border-white/15 bg-[#181412]/88 shadow-[0_30px_70px_rgba(20,15,12,0.3)] backdrop-blur-xl ${condensed ? "p-5" : "p-6 md:p-7"}`}>
      ...
      <div className="relative">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <span className="rounded-full bg-[#f2eee8] px-3 py-1.5 text-xs font-black text-[#33271f]">שלושת המוצרים יחד</span>
          <span className="text-xs font-medium text-white/60">תשלום חד־פעמי · ללא מנוי</span>
        </div>
        {!condensed ? <div className="mt-4 rounded-2xl border border-[#efc8b6]/24 bg-[#5c261d]/38 p-4"><div className="mb-3 flex items-center justify-center gap-2 text-xs font-black text-[#f2ddd2]"><span className="h-2 w-2 animate-pulse rounded-full bg-[#ff9b76]" />ההטבה נסגרת בעוד</div><Countdown /></div> : null}
        <div className="mt-5 grid grid-cols-2 gap-2 text-center md:gap-3">
          <div className="rounded-2xl border border-white/10 bg-white/[0.05] px-1 py-3"><div className="text-[10px] font-bold text-white/45">השווי המקורי הכולל</div><div className="mt-1 text-base font-black text-white/65 line-through md:text-lg">1,245 ₪</div></div>
          <div className="rounded-2xl bg-[#f4efe8] px-1 py-3 text-[#2c211a] shadow-[0_12px_28px_rgba(220,205,190,0.18)]"><div className="text-[10px] font-black">מחיר ההטבה</div><div className="mt-1 text-2xl font-black md:text-3xl">399 ₪</div></div>
        </div>
        <p className={`text-center font-black leading-5 text-[#e5cdbd] ${condensed ? "mt-3 text-xs" : "mt-4 text-sm"}`}>חיסכון של 846 ₪ · 68% הנחה</p>
      </div>
    </div>
  );
}
...
export default function NewYearLoveBundle() {
  ...
  useEffect(() => {
    ...
    const description = "המאגר הוא הלב. המדריך לבחור נכון והקורס המסע לזוגיות עוזרים להגיע להיכרות מוכנים יותר. שלושתם בהטבה שלא הייתה כאן מעולם: 399 ₪ במקום שווי מקורי כולל של 1,245 ₪.";
      ...
      <header className="relative isolate min-h-[900px] overflow-hidden bg-[#1d1916] px-5 pb-20 pt-28 text-white md:min-h-[790px] md:px-8 md:pb-24 md:pt-32">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.02fr_0.98fr] lg:gap-16">
          <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.68 }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#efc8b6]/35 bg-[#8a3828]/30 px-4 py-2 text-xs font-black text-[#fff4ed] backdrop-blur-sm"><Flower2 className="h-3.5 w-3.5" /> הטבה שלא הייתה כאן מעולם · עד 30.9.2026</div>
            <h1 className="max-w-3xl text-[2.8rem] font-black leading-[1.02] tracking-[-0.045em] text-white md:text-6xl lg:text-7xl">
              המאגר הוא הלב.
              <span className="mt-2 block text-[#e3d2c1]">המדריך והקורס</span>
              <span className="mt-2 block">מכינים אתכם אליו.</span>
            </h1>
            <p className="mt-7 max-w-2xl text-lg font-bold leading-8 text-[#f6f0ea]/92 md:text-2xl md:leading-9">כדי להגיע להיכרות מוכנים יותר, חיברתי למאגר את המדריך ״לבחור נכון״ ואת הקורס ״המסע לזוגיות״ בהטבה מיוחדת של שלושתם ב־399 ₪.</p>
            <p className="mt-4 max-w-xl text-base leading-7 text-white/78 md:text-lg">ואני שמה עכשיו גז על ההתאמות, כדי שהחגים השנה באמת יוכלו להיראות אחרת.</p>
      ...
      <main className="pb-24 md:pb-0">
        ...
        <section className="relative bg-[#f9f6f1] px-5 py-20 md:px-8 md:py-28">
          <Reveal className="mx-auto max-w-7xl">
            <motion.div variants={fadeUp} className="mx-auto max-w-4xl text-center"><p className="text-sm font-black text-[#806650]">למה שלושתם יחד</p><h2 className="mt-3 text-3xl font-black leading-tight tracking-[-0.035em] text-[#29211c] md:text-5xl">המאגר פותח את ההזדמנות. המדריך והקורס עוזרים להגיע אליה מוכנים.</h2><p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-[#68594f]">כל אחד מהם נותן חלק אחר: אנשים חדשים להכיר, בחירה מדויקת יותר ועבודה עמוקה על הדפוסים שמנהלים את הדרך.</p></motion.div>
        ...
        <section ref={paymentRef} id="payment" className="relative isolate overflow-hidden bg-[#1c1815] px-5 py-20 text-white md:px-8 md:py-28"><FlowerField /><Reveal className="relative mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:gap-16"><motion.div variants={fadeUp} className="text-center lg:text-right"><div className="inline-flex items-center gap-2 rounded-full border border-[#efc8b6]/35 bg-[#8a3828]/30 px-4 py-2 text-xs font-black text-[#fff4ed]"><Clock3 className="h-4 w-4" /> הטבה שלא הייתה כאן מעולם</div><h2 className="mt-7 text-4xl font-black leading-tight tracking-[-0.04em] md:text-5xl">המאגר הוא הלב. המדריך והקורס מכינים אתכם אליו.</h2><p className="mt-5 text-lg leading-8 text-white/72">שלושת המוצרים יחד ב־399 ₪ במקום שווי מקורי כולל של 1,245 ₪.</p><div className="mt-7"><PricePanel condensed /></div></motion.div><motion.div variants={fadeUp} className="rounded-[2rem] border border-white/13 bg-[#f9f6f1] p-6 text-[#29211c] shadow-[0_30px_85px_rgba(10,7,5,0.44)] md:p-9"><p className="text-sm font-black text-[#806650]">הצטרפות לחבילת החג</p><h3 className="mt-2 text-3xl font-black tracking-[-0.03em]">שלושתם ב־399 ₪</h3>{newsletterCoupon ? <div className="mt-3 rounded-2xl border border-[#d9bea2] bg-[#efe1d2] px-4 py-3 text-sm font-bold text-[#5c3824]">קוד HOLIDAY10 מהניוזלטר יחכה כאן ויופעל לאחר הזנת המייל.</div> : null}<div className="mt-5 space-y-2 rounded-2xl bg-[#eee7df] p-4 text-sm font-bold text-[#4b3b31]">{["מאגר הרווקים והרווקות", "המדריך ״לבחור נכון״", "הקורס ״המסע לזוגיות״"].map((item) => <div key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-[#806650]" />{item}</div>)}</div><p className="mt-4 text-sm leading-6 text-[#6a5a50]">לאחר אישור Grow יישלחו קישורי הגישה למדריך ולקורס, ויתחיל תהליך ההצטרפות למאגר.</p><div className="mt-6"><GrowWallet product="bundle_new_year" prefillCoupon={newsletterCoupon} buttonLabel="אני רוצה את שלושתם ב־399 ₪" buttonClassName="!bg-[#29211c] !text-white !font-black !text-lg !rounded-full hover:!bg-[#3a2e27] !py-4" termsPath="/terms/new-year-love" onSuccess={() => { window.location.href = "/thank-you/new-year-love"; }} /></div><div className="mt-5 flex items-center justify-center gap-4 text-center text-[11px] font-medium text-[#6f6056]"><span className="inline-flex items-center gap-1"><ShieldCheck className="h-3.5 w-3.5" /> תשלום מאובטח</span><span className="inline-flex items-center gap-1"><Check className="h-3.5 w-3.5" /> ללא חיוב מתחדש</span></div><p className="mt-4 text-center text-[11px] leading-5 text-[#82736a]">תנאי הביטול והגישה מפורטים בתקנון החבילה.</p></motion.div></Reveal></section>
