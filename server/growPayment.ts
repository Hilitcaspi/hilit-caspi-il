...
const PAGE_CODES: Record<string, string> = {
  ...
  plus:         process.env.GROW_SANDBOX_RECURRING_PAGE_CODE || "",
};
...
export const PRODUCT_CONFIGS: Record<string, ProductConfig> = {
  ...
  plus:         { description: "Database Plus - מנוי חודשי",                         sum: 1 },
};
...
export async function createPaymentProcess(input: CreatePaymentInput): Promise<CreatePaymentResult> {
  ...
  const isSandboxPlus = input.product === "plus";
  const pageCode = PAGE_CODES[input.product];
  const growUserId = isSandboxPlus ? process.env.GROW_SANDBOX_USER_ID || "" : GROW_USER_ID;
  const growApiUrl = isSandboxPlus
  ...
  if (isSandboxPlus) {
    console.info("[GrowPayment] Plus Sandbox request", {
      sandboxUrl: growApiUrl.includes("sandbox.meshulam.co.il"),
      userIdMatchesConfiguredPair: growUserId === "10be6655a4711b2a",
      pageCodeMatchesConfiguredPair: pageCode === "18c95ad6faf7",
      amount: sum,
      recurringChargeType: 1,
    });
    ...
    const response = await globalThis.fetch(growApiUrl, { method: "POST", body: form, headers: { Accept: "application/json" } });
    const json = await response.json() as any;
    if (!response.ok || json?.status !== 1 || !json?.data?.url) throw new Error(json?.err || "Grow Sandbox did not return a hosted payment form");
    return { authCode: "", processToken: json.data.processToken, url: json.data.url };
  }
  ...
  let res: Response;
  try {
    res = await globalThis.fetch(growApiUrl, {
      method: "POST",
