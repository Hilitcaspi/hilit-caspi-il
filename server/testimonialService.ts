...
export function buildTestimonialDraft(input: {
  ...
  if (input.sourceType === "match") {
    return {
      subject: `${firstName}, אשמח לשמוע על החוויה שלך מההתאמה ולהעניק לך מתנה אישית ממני`,
      body: `היי ${firstName}, שמחתי לראות שבהתאמה שלך הייתה הסכמה משני הצדדים. אשמח לשמוע בכמה מילים לא רק על ההיכרות עצמה, אלא גם על הדרך, הבחירה והחוויה שקיבלת מהמאגר וממני. המלצה אמיתית שלך יכולה לעזור לעוד אנשים שמחפשים אהבה להכיר את הפלטפורמה, להצטרף למאגר ולהגדיל את מספר ההזדמנויות וההתאמות עבור כולם. בסיום מחכה לך מתנה אישית ממני. המתנה ניתנת על עצם השיתוף, גם בלי אישור לפרסם. רק אם מתאים לך, אפשר לבחור בנפרד מה מותר לנו לשתף, היכן ובאיזו זהות.`,
    };
  }
  if (input.sourceType === "dna") {
    return {
      subject: `${firstName}, אשמח לשמוע מה לקחת משאלון ה־DNA ולהעניק לך מתנה אישית ממני`,
      body: `היי ${firstName}, אחרי שסיימת את שאלון ה־DNA הזוגי וקיבלת את התוצאה האישית, אשמח לשמוע איזו תובנה לקחת ממנו ומה הרגיש לך מדויק או משמעותי. כמה מילים שלך יכולות לעזור לעוד אנשים שמחפשים אהבה להכיר כלי שייתן להם הבנה חדשה על עצמם ועל הבחירות שלהם. בסיום מחכה לך מתנה אישית ממני. המתנה ניתנת על עצם השיתוף, גם בלי אישור לפרסם. רק אם מתאים לך, אפשר לבחור בנפרד מה מותר לנו לשתף, היכן ובאיזו זהות.`,
    };
  }
  return {
