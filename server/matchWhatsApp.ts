...
export function buildMatchWhatsAppMessage(firstName: string, matchFirstName: string, score: number, proposalSource: "manual" | "boost" = "manual", boostRole: BoostRecipientRole = "recipient"): string {
  if (proposalSource === "boost") {
    if (boostRole === "sender") {
      return `היי ${firstName}\n\nבקשת ה־Boost שלך, עם ${score}% התאמה, נשלחה לשני הצדדים. התשלום ושליחת ה־Boost אינם אישור להתאמה.\n\nמייל עם התמונה, פרטי ההתאמה וכפתורי האישור מחכה לך. כדאי לבדוק גם בספאם ובתיקיית השיווק ולאשר או לדחות במייל.\n\nצוות הילית כספי 💛`;
    }
    return `היי ${firstName}\n\nנשלחה אליך התאמת Boost מיוחדת עם ${score}% התאמה. היא נוצרה על ידי אלגוריתם ההתאמה במסגרת מסלול Boost, ולא נבחרה או נבדקה אישית על ידי הילית.\n\nמייל עם התמונה, פרטי ההתאמה וכפתורי האישור מחכה לך. כדאי לבדוק גם בספאם ובתיקיית השיווק ולאשר או לדחות במייל.\n\nצוות הילית כספי 💛`;
  }
